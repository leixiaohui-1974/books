...
1. SCADA 与 HDC 在胶东调水中的核心分工是什么？
2. 为什么指令执行必须绑定策略版本与审计流水号？
3. 简述自动/协同/人工三种模式的适用场景。

### 应用题

4. 设计一份“SCADA字段—模型变量”映射模板，至少包含 8 个字段。  
5. 给出一个联锁降级流程图草案，覆盖触发、降级、恢复三阶段。

### 思考题

6. 在供水紧约束条件下，若自动模式频繁切换到协同模式，系统治理上应优先改进哪一层？

### 基础题参考答案要点

1. SCADA负责执行与监控，HDC负责协调与决策。  
2. 便于追责、复盘与版本一致性管理。  
3. 自动适合常态，协同适合不确定状态，人工适合极端与应急。
...
> **术语一致性声明**：本章统一采用“分层分布式控制（Hierarchical Distributed Control, HDC）”“运行设计域（Operational Design Domain, ODD）”“安全包络（Safety Envelope）”“在环测试（In-the-Loop Testing）”“模型预测控制（Model Predictive Control, MPC）”等规范术语；数学符号采用 $Q,h,\tau,T_p,T_c,\mathbf{x},\mathbf{u},\mathbf{y},J$。
...
- [x] 图表不少于2项（图4-1、表4-1）；
- [x] 章首回顾ch03，章末预告ch05；
- [x] 已补齐ch03对ch04的接口字段与模型变量映射跟踪项（含TODO占位）。
