...
**10-1.** MIL、SIL、HIL 三者的核心差异是什么？  
**10-2.** 为什么在环测试必须关注“风险覆盖率”而不只是“用例数量”？  
**10-3.** P0 用例与 S0 缺陷分别代表什么管理含义？

### 基础题参考答案要点

**10-4.** MIL验证模型与算法逻辑，SIL验证软件集成与接口时序，HIL验证真实硬件约束下的闭环行为。  
**10-5.** 因为同样数量的用例可能遗漏高风险场景；风险覆盖率用于衡量关键失效模式是否被验证。  
**10-6.** P0是必须全通过的关键安全用例等级；S0是必须立即阻断发布的致命缺陷等级。

### 应用题

**10-7.** 为“泵站群协同控制升级”设计一套两周在环测试计划。  
**10-8.** 设计一个版本发布门禁看板（至少包含5项核心指标）。

### 思考题

**10-9.** 当测试周期受限时，如何平衡“覆盖深度”与“发布时效”？
