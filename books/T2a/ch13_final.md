...
1. 数字孪生与普通离线仿真的核心差异是什么？  
2. FMI 与 FMU 分别解决什么问题？  
3. 为什么在线参数更新必须配套灰度发布与回滚机制？

> **术语一致性声明**：本章统一采用“数字孪生（Digital Twin）”“在环测试（In-the-Loop Testing）”“模型预测控制（Model Predictive Control, MPC）”“故障检测与隔离（FDI）”“SCADA+MAS Fusion Architecture”等规范术语，并与前序章节术语体系保持一致。

### 基础题参考答案要点

1. 数字孪生强调与现场实时数据同步和在线闭环更新，离线仿真主要用于静态方案验证。  
2. FMI定义跨工具互操作接口标准，FMU是按该标准封装的可执行模型单元。  
3. 防止在线改参引发运行风险，确保参数变更可验证、可回滚、可审计。

### 应用题

4. 为某明渠调水系统设计一套“影子孪生+灰度发布”流程。  
5. 设计一个包含传感器缺测与通信延迟的孪生回归测试集。

### 思考题

6. 当“模型精度”与“平台实时性”冲突时，如何给出工程可接受折中？
