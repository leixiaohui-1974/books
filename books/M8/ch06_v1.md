...
## 6.5 在环结果到发布门禁映射（补齐ch05/ch04跟踪项）
...
1. 为什么在环测试不能被离线仿真完全替代？
2. 解释 $C_{itl}$ 三个组成部分的工程意义。
3. HIL 关键失败为何应触发强制阻断？

### 应用题

4. 设计一套发布门禁映射表，包含至少 5 条“测试结果→门禁动作”规则。  
5. 给出一个异常注入回归计划，覆盖传感器、通信、执行器三类故障。

### 思考题

6. 如果覆盖度达标但线上仍出现异常，问题更可能出在测试设计还是运行环境，如何判定？

### 基础题参考答案要点

1. 在环能验证真实时序与软硬件耦合行为。  
2. 分别对应场景完整性、约束边界覆盖与故障韧性。  
3. 因联锁直接关系运行安全，容错空间极小。
...
> **术语一致性声明**：本章统一采用“在环测试（In-the-Loop Testing）”“运行设计域（Operational Design Domain, ODD）”“安全包络（Safety Envelope）”“分层分布式控制（Hierarchical Distributed Control, HDC）”“模型预测控制（Model Predictive Control, MPC）”等规范术语；数学符号采用 $Q,h,\tau,T_p,T_c,\mathbf{x},\mathbf{u},\mathbf{y},J$。
...
- [x] 图表不少于2项（图6-1、表6-1）；
- [x] 章首回顾ch05，章末预告ch07；
- [x] 已补齐ch05对ch06的在环覆盖度与发布门禁映射跟踪项（含TODO占位）。
