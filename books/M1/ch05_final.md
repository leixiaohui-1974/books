...
1. $K$ 反映波传播时间尺度，$X$ 反映平移与扩散权重分配。  
2. 该区间可保证模型具有合理扩散特性并减少非物理振荡风险。  
3. 系数和为1体现离散质量守恒一致性，避免系统性偏差累积。

> **术语一致性声明**：本章统一采用“降阶模型（Reduced-Order Model, ROM）”“传递函数（Transfer Function）”“IDZ（Integrator Delay Zero）模型”“运行设计域（Operational Design Domain, ODD）”“安全包络（Safety Envelope）”“模型预测控制（Model Predictive Control, MPC）”“在环测试（In-the-Loop Testing）”等规范术语；数学符号使用 $Q, h, A, S_0, S_f, n, c, \tau, G(s)$ 等统一记号。

### 质量检查清单（执行结果）

- [x] 章首学习目标、章末小结、习题、拓展阅读完整；  
- [x] 基础题3 + 应用题2 + 思考题1，且补充基础题答案要点；  
- [x] 章首回顾ch04结论，章末预告ch06内容；  
- [x] 每组核心公式均配物理直觉/工程解释；  
- [x] 图表占位不少于2项（图5-1、图5-2、表5-1）；  
- [x] 包含可复现实例（例5-1）与参数率定流程。
