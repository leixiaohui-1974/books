...
**2-1.** 写出一维明渠 Saint-Venant 连续方程与动量方程，并说明各项物理意义。  
**2-2.** 为什么亚临界流需要上下游双边界条件？  
**2-3.** 在特征线法中，$C^+$ 与 $C^-$ 两族线分别代表什么信息传播方向？


### 基础题参考答案要点

**2-4.** 连续方程描述存量守恒，动量方程描述驱动项与阻力项平衡；两式共同决定 $Q$ 与 $h$ 的时空演化。  
**2-5.** 因为亚临界流存在上下游双向信息传播，单侧边界不足以唯一确定内部状态。  
**2-6.** $C^+$ 通常对应顺流向传播分量，$C^-$ 对应逆流向传播分量，二者共同定义状态更新。

### 应用题

**2-7.** 某渠段 $L=8\,\text{km}$，$u_0=0.6\,\text{m/s}$，$c=2.4\,\text{m/s}$。估算下游主导响应传播时间，并讨论若旁侧入流不可忽略该估计会如何偏移。  
**2-8.** 设计一个“边界小扰动试验方案”，用于识别 $\tau$ 与主导时间常数 $T$，需给出采样周期、观测变量与误差评价指标。

### 思考题

**2-9.** 在冰期或高含沙条件下，Manning 粗糙系数时变明显。此时应如何修改“线性化+特征线”分析流程以保持 ROM 参数有效性？
