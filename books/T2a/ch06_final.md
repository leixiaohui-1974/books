<!-- 变更日志
v2 2026-02-16: 根据四角色评审修订——补充术语一致性声明与基础题答案要点，强化可教性
v1 2026-02-16: 初稿
-->
...
**6-1.** 解释 \(\mathbf{A},\mathbf{B},\mathbf{C}\) 矩阵在工程中的含义。  
**6-2.** LQR 中 \(\mathbf{Q}\) 与 \(\mathbf{R}\) 权重分别影响什么？  
**6-3.** 为什么 LQG 需要估计器与控制器协同设计？

### 基础题参考答案要点

**6-4.** \(\mathbf{A}\) 描述系统内在动态，\(\mathbf{B}\) 描述输入作用链路，\(\mathbf{C}\) 描述可测映射。  
**6-5.** \(\mathbf{Q}\) 强调状态性能，\(\mathbf{R}\) 约束控制动作强度。  
**6-6.** 因状态不可全测且含噪声，需估计器提供可用状态供反馈控制使用。

### 应用题

**6-7.** 为某泵站系统设计一套 LQR 初始整定策略，并说明如何在在环测试中修正。  
**6-8.** 在参数不确定性显著场景下，比较 LQR 与 H∞ 的上线策略差异。

### 思考题

**6-9.** 当自适应控制与安全约束发生冲突时，应如何设置“学习速度”与“安全优先级”？
