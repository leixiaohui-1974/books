...
**7-1.** 为什么数据驱动 ROM 必须配合 ODD 与 Safety Envelope 使用？  
**7-2.** 子空间法与残差学习法的主要差异是什么？  
**7-3.** 数据泄漏会对验证结果造成什么影响？

### 基础题参考答案要点

**7-4.** 因为数据模型存在外推风险，需通过 ODD 与 Safety Envelope 约束其适用边界与失效回退。  
**7-5.** 子空间法侧重状态降维与主模态保持；残差学习法侧重补偿机理模型误差。  
**7-6.** 会高估模型性能，导致上线后误差显著反弹。

### 应用题

**7-7.** 某模型 MAE 从 0.050 m 降至 0.034 m，推理时延从 10 ms 升至 22 ms。请评估上线可行性并说明依据。  
**7-8.** 设计一个“模型漂移监测看板”指标集，至少包含精度、分布漂移、告警触发与回退频次。

### 思考题

**7-9.** 在高风险调度场景下，你会优先选择“高精度黑箱模型”还是“中等精度可解释模型”？请给出工程决策原则。
