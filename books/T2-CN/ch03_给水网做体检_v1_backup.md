...
**第三步：查可控性。** 对照每一个控制目标，问：有执行器（闸门/泵站/阀门）能影响到它吗？影响的时间延迟有多大？控制力度够不够？如果延迟超过了容许的响应时间——这就是"死区"，标红。还要检查执行器的可靠性：闸门的液压系统上次维护是什么时候？泵站的备用机组能不能正常启动？很多工程的"可控性"在纸面上是够的，但实际上因为设备老化、维护不到位，真到用时可能失灵。
...
体检报告不仅告诉你"哪里有问题"，还间接告诉你"你的工程在WNAL哪一级"（下一章会详细讲WNAL分级）。一个粗略的对应关系：
...
短期能做的是"用信息换时间"——利用预测控制（MPC）技术，提前预判来水变化，在问题还没传播到"死区"之前就在上游提前采取行动。打个比方：你的车刹车距离很长（可控性弱），那你就必须看得更远（用预测弥补），提前减速。沙坪水电站就是用这个思路——调节库容极小（"刹车距离"短），但用MPC预测未来70分钟的来水变化，提前调整闸门，相当于"把刹车提前踩了"。
