...
本章完成了沙坪“认知智能+ODD+HDC”控制设计，补齐了 ODD 切换条件、门禁状态机、认知 AI 解释变量模板和人机切换流程，为梯级一键调提供统一控制与治理接口。

下一章将整合前述单站控制策略，构建沙坪梯级水电站群的多站联合优化调度架构。

## 习题

### 基础题

**10-1.** ODD-G、ODD-Y、ODD-R 的核心差异是什么？
**10-2.** 说明 `release_gate` 三种状态的权限边界。
**10-3.** 为什么认知 AI 必须输出解释变量而非仅输出告警等级？

### 应用题

**10-4.** 设计一个包含 6 个指标的门禁评分模型并给出阈值划分。  
**10-5.** 基于一次通信异常，写出“自动降级→人工接管→恢复发布”的流程图。

### 思考题

**10-6.** 当认知 AI 建议与值班工程师判断冲突时，如何在效率与责任之间平衡？

### 基础题参考答案要点

**10-7.** 差异在于工况可控性与自动控制权限：G可全自动，Y受限自动，R需最小风险运行与人工增强。  
**10-8.** `OPEN`可自动发布，`LIMITED`需限制或确认，`CLOSED`禁止新策略仅保安全动作。  
**10-9.** 因工程决策需要证据链，解释变量支持追责、复盘与模型迭代。
