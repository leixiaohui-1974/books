...
**3-1.** 为什么运行过程模型需要同时包含扰动项与噪声项？
**3-2.** 说明“三元混合模型”中规则层的作用。
**3-3.** 写出误差分级阈值用于运行门禁的基本逻辑。

### 应用题

**3-4.** 结合一个渠道或泵站场景，给出四类不确定性的实例并标注可能责任角色。  
**3-5.** 设计一个“连续超阈值触发”判据（给出 $N$ 与阈值建议）并说明理由。

### 思考题

**3-6.** 在“可解释但保守”和“高性能但黑箱”两类模型之间，运行团队应如何选择？请结合治理目标作答。

### 基础题参考答案要点

**3-7.** 扰动表示外界驱动变化，噪声表示不可避免的测量/过程随机误差，两者来源不同。  
**3-8.** 规则层把模型输出转化为可执行门禁条件，保证策略合法、可审计。  
**3-9.** 先按误差区间分级，再绑定自动与人工动作，保证响应一致性。
