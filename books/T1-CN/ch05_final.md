...
**胶东调水工程**：干线全长 440km，穿越 5 个泵站加压区段。按分区格拉姆分析，每区独立识别约 8–12 个关键测点，全线共部署水位传感器 52 个（主测）+ 11 个（备用/验证），通过 EKF 状态估计器实现全线可观，控制精度 RMSE < 3 cm。特别值得指出的是，胶东调水冬季存在冰期工况：渠道糙率系数升高 10%–25%，传播时延延长 8%–15%，需在 EKF 中实时辨识参数以维持估计精度（第 14 章详述在线辨识方案）。
...
基于 IDZ 传递函数对 20 个渠池离散化，建立线性状态空间模型：
...
**与第 6 章的衔接**：第 6 章将以本章的工程判据为基础，深入阐述八原理中"原理二：可控可观性"的理论来源和系统设计含义，将工程实施层面的"怎么做"提升为学科方法论层面的"为什么这样做"。两章之间的关系，正如施工规范与设计规范：本章是操作层，第 6 章是原理层，相互支撑、缺一不可。
...
**核心认识**：本章的可控可观性理论与 HydroOS 架构之间不是"理论"与"实现"的单向关系，而是双向印证——可控可观性分析为 HydroOS 的传感器配置和控制器设计提供数学依据，HydroOS 的运行数据反过来可以验证和修正可控可观性分析的假设（如模型参数漂移的定量评估）。第 12–14 章的工程案例将在具体工程背景下进一步展示这种双向印证关系。
...
**与后续章节的衔接**：本章建立的可控可观性工程框架是后续章节的重要铺垫。第 6 章八原理中的"原理二（可控可观性）"将从学科理论层面阐释本章工程判据的思想来源；第 8 章安全包络与在环验证将以本章的三级验证框架（MIL/SIL/HIL）为基础，详细展开验证场景设计和评判准则；第 12–14 章的三个工程案例将在南水北调、大渡河梯级、胶东调水等具体工程背景下，展示本章理论与方法的完整应用过程。
