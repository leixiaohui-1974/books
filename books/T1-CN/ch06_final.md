...
本章在《水系统控制论》框架中承接第四章形式化描述，为第七章八原理中的"原理二：可控可观性"提供严格的工程实施指南。水系统之所以难以实现高等级自主运行，根本障碍往往不是控制算法不够先进，而是系统本身的可控可观性不足——传感器布局不当导致关键状态"看不到"，执行器配置不当导致控制指令"不生效"。本章的核心问题是：怎样定量判断一个实际工程的可控可观性？如何基于这些判据设计传感器和执行器的最优配置？

可控可观性不是"装了设备就有"的属性，而是由系统动力学、执行器约束和传感器位置共同决定的内在特性。本章将Kalman的经典数学判据扩展为工程判据，建立从理论验证、工程规则到实践优化的完整分析框架，为第八章在环验证提供必要的理论基础。特别地，可控可观性的满足程度直接影响系统能否进入更高等级的自主运行模式——若关键状态在任何时刻都无法被观测或估计，系统的感知与决策链条必将断裂。
...
### 6.1.3 可控性与 水网自主等级（WNAL） 等级的关系

可控性是实现 水网自主等级（WNAL） 高等级自主运行的前提条件。不同 WNAL 等级对可控性的要求如下：

**表6-1：WNAL等级与可控可观性要求对应关系**
...
| L5（完全自主） | 全场景可控 | 在所有 运行设计域（ODD） 场景下均保持可控性 |
...
| L5（完全自主） | 全场景可观 | 在所有 运行设计域（ODD） 场景下均保持可观性 |
...
传感器布局优化的目标是在满足可观性要求的前提下（基于第四章的状态空间形式化模型），最小化总成本（投资 + 运维）：
...
| 参数 | 典型值 | 不确定性范围 | 主要原因 |
|-----|-------|----------|---------|
| 渠道糙率 $n$ | 0.015–0.025 | ±20–40% | 季节性藻类生长、淤积变化、温度影响 |
| 蓄水面积 $A_s$ | 500–50,000 m² | ±10–15% | 断面形状变化、淤积测量误差 |
| 纯时延 $\tau$ | 600–7200 s | ±10–25% | 流量变化导致波速变化、边界条件简化 |
| 零点参数 $p_z$ | 100–2000 s | ±30–50% | 复杂边界条件、非线性效应 |
| 泵机效率 $\eta$ | 0.75–0.95 | ±5–15% | 设备老化、运行工况偏移 |
...
**工程后果**：MPC 控制器基于错误模型参数运行，预测精度下降，控制输出出现振荡或过调。长距离输水工程冬季运行时此问题尤为突出（如第十五章冰期参数在线辨识方案所述）。
...
跨流域输水工程涉及数百公里管渠、数十座泵闸、数百个测量点，直接对完整系统进行可控可观性矩阵分析在计算上不可行（状态维度 $n > 500$ 时矩阵操作耗时指数增长）。本节介绍三种工程可行的分解方法。
...
**与后续章节的衔接**：本章建立的可控可观性工程框架是后续章节的重要铺垫。第十一章八原理中的"原理二（可控可观性）"将从学科理论层面阐释本章工程判据的思想来源；第十一章安全包络与在环验证将以本章的三级验证框架（MIL/SIL/HIL）为基础，详细展开验证场景设计和评判准则；第十二至十四章的工程案例将展示本章理论与方法的完整应用过程。
...
[6-1] Kalman R E. On the general theory of control systems [C]//Proceedings of the First International Congress on Automatic Control. London: Butterworth, 1960: 481-492.

[6-2] Evensen G. Data Assimilation: The Ensemble Kalman Filter [M]. 2nd ed. Berlin: Springer, 2009.

[6-3] Hautus M L J. Controllability and observability conditions of linear autonomous systems [J]. Proceedings of the Netherlands Academy of Science, 1969, 72: 443-448.

[6-4] Müller P C, Weber H I. Analysis and optimization of certain qualities of controllability and observability for linear dynamical systems [J]. Automatica, 1972, 8(3): 237-246.

[6-5] Moore B C. Principal component analysis in linear systems: Controllability, observability, and model reduction [J]. IEEE Transactions on Automatic Control, 1981, 26(1): 17-32.

[6-6] 王浩，雷晓辉，尚毅梓。南水北调中线工程智能调控与应急调度关键技术 [J]. 南水北调与水利科技，2017, 15(2): 1-8.

[6-7] 孔令仲，雷晓辉，许明。调水工程传感器优化布局方法 [J]. 水力发电学报，2022, 41(3): 78-89.

[6-8] Cover T M, Thomas J A. Elements of Information Theory [M]. 2nd ed. Hoboken, NJ: Wiley, 2006.
