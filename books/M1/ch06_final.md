...
本章建立了频率域辨识在明渠 ROM 体系中的方法框架：从 FRF 估计、相干筛选到参数提取与工程门禁，形成“频域提取 + 时域验证”的双域闭环。其核心结论是：频率域辨识能显著提升参数初值质量与频段可解释性，但不能脱离 ODD、Safety Envelope 与在环测试（In-the-Loop Testing）单独使用。下一章将进入数据驱动降阶方法，讨论如何在保留机理边界的同时引入学习能力。

## 习题

### 基础题

**6-1.** 为什么频率域辨识不能完全替代时域辨识？  
**6-2.** 相干函数在频域辨识中起什么作用？  
**6-3.** Bode 图中相位线性下降通常对应什么物理量？

### 基础题参考答案要点

**6-4.** 频域辨识擅长频段特性提取，但闭环性能仍需时域验证；两者关注点不同且互补。  
**6-5.** 相干函数用于衡量输入输出在各频段的线性相关与数据可信度。  
**6-6.** 常对应纯滞后（传输延时）主导的相位贡献。

### 应用题

**6-7.** 某系统在低频增益为 -6 dB，拐点频率为 $8\times10^{-4}\,\text{rad/s}$，延时估计 240 s。写出一阶滞后候选模型。  
**6-8.** 给定一组 FRF 数据与相干曲线，设计“可上线/不可上线”的判据，并说明阈值依据。

### 思考题

**6-9.** 当频域拟合优良但在环测试性能一般时，你会优先调整模型结构、控制器带宽还是传感器配置？请给出工程决策逻辑。
