...
1. 为什么MPC实时部署必须关注P95/P99而非仅平均时延？
2. 写出控制周期内时延分解公式并解释各项含义。
3. 超时降级为什么是“控制稳定性机制”而非“性能妥协”？

### 应用题

4. 某系统 $T_c=90$ s，给定感知/通信/执行预算分别为10/15/10 s，取 $\gamma=0.75$，计算求解预算与告警阈值，并设计三级处置动作。  
5. 结合你所在工程，设计一份“求解器参数A/B/C切换表”，包含触发条件与恢复条件。

### 思考题

6. 在极端洪水场景下，若“高精度优化”与“及时可行解”冲突，应如何设定审批优先级与责任边界？

### 基础题参考答案要点

1. 尾部时延决定超时风险，直接影响安全门禁。  
2. $\tau_{sense}+\tau_{comm}+\tau_{solve}+\tau_{act}\le T_c$，分别对应采集、通信、求解、执行。  
3. 降级保障连续可行控制，避免无控制输出导致风险放大。
