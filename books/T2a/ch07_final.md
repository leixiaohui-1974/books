...
1. 说明 \(T_p\) 与 \(T_c\) 的区别及其工程意义。  
2. 为什么MPC每次只执行最优序列的第一步？  
3. 解释 \(\mathbf{Q}\) 与 \(\mathbf{R}\) 在目标函数中的作用。
...
1. \(T_p\) 是预测窗口，\(T_c\) 是优化控制窗口；前者偏向看得远，后者偏向算得动。  
2. 仅执行第一步可利用下一时刻新测量滚动纠偏，增强抗扰与鲁棒性。  
3. \(\mathbf{Q}\) 强调跟踪性能，\(\mathbf{R}\) 抑制控制动作幅度与抖动。

### 应用题

4. 为某双闸门渠段给出线性MPC最小化问题（含约束集合）并说明参数初值。  
5. 某场景中求解器偶发超时，请提出两类工程修正策略并比较优缺点。

### 思考题

6. 在“预测精度提升”和“求解实时性保障”冲突时，你会如何制定上线优先级？
