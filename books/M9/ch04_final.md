...
1. 为什么运行决策问题必须同时写出目标函数与约束集合？
2. 解释“可行域为空”在工程上意味着什么。
3. 权重敏感性指标 $S_{w_j}$ 的作用是什么？

### 应用题

4. 为一个泵站+水库联合场景设计权重组合，并说明在枯水与丰水工况下如何切换。  
5. 依据表4-2模板，编制一份不少于10项的本单位策略发布门禁清单。

### 思考题

6. 若管理层要求“效率优先”，但运行团队判定安全裕度不足，应如何在治理层面达成一致？

### 基础题参考答案要点

1. 只有目标没有约束会导致不可执行，只有约束没有目标无法比较优劣。  
2. 表示现有条件下无任何策略可同时满足底线要求，必须降级或重设目标。  
3. 用于判断最优策略对权重变化是否稳定，辅助制定稳健权重方案。
...
> **术语一致性声明**：本章统一采用“运行设计域（Operational Design Domain, ODD）”“安全包络（Safety Envelope）”“最小风险状态（Minimal Risk State, MRS）”“模型预测控制（Model Predictive Control, MPC）”“水网操作系统（Water Network Operating System, HydroOS）”等规范术语；数学符号采用 $Q,h,\tau,T_p,T_c,\mathbf{x},\mathbf{u},\mathbf{y},J$。
...
- [x] 公式前有物理直觉，公式后有工程解释；
- [x] 含可复现实例（例4-1）；
- [x] 图表不少于2项（图4-1、图4-2、表4-1、表4-2）；
- [x] 章首回顾ch03，章末预告ch05；
- [x] 已补齐ch03评审提出的权重敏感性示例、可行域解释图与门禁检查单模板。
