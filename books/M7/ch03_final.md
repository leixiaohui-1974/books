...
**3-1.** 为什么 DAL 是 HydroOS 的“变化隔离层”？
**3-2.** 统一资源模型中为什么必须包含 `quality_code` 与 `timestamp`？
**3-3.** 解释 DAL 时序预算公式各项含义。

### 应用题

**3-4.** 针对一个含 OPC UA 与 Modbus 混合站点，设计适配器健康检查指标与告警阈值。  
**3-5.** 设计一份“协议适配上线验收单”，至少包含功能、性能、异常、安全四类判定项。

### 思考题

**3-6.** 若现场存在无法升级的老旧设备，你如何在保证安全前提下纳入 HydroOS 统一调度？

### 基础题参考答案要点

**3-7.** 它隔离设备协议差异，避免上层应用直接耦合现场异构细节。  
**3-8.** 质量位与时间戳决定数据是否可信、是否可用于实时控制。  
**3-9.** 分别对应采集、标准化、分发时延，总和应受接入预算约束。
