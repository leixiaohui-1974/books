...
**9-1.** 说明 HydroOS 三层 API 的职责边界与典型调用关系。  
**9-2.** 区分 `API-major/minor/patch` 三类变更的兼容性影响。  
**9-3.** 为什么契约测试应先于服务实现？

### 应用题

**9-4.** 设计一份调水业务 API 的版本治理策略，包含变更分级、测试门槛与审批流程。  
**9-5.** 为“伙伴插件”场景制定最小隔离策略（运行、数据、审计三个维度）。

### 思考题

**9-6.** 在多单位协同的流域调度中，如何平衡“开放生态接口”与“控制系统安全闭环”？

### 基础题参考答案要点

**9-7.** 设备层管抽象控制点，能力层管算法与服务，业务层管流程协同。  
**9-8.** major 破坏兼容，minor 向后兼容扩展，patch 非语义修复。  
**9-9.** 契约优先可前置协同问题，降低集成与回归成本。
