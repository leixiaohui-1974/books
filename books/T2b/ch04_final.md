...
**4-1.** PINN 与普通神经网络的核心差异是什么？  
**4-2.** 为什么 PINN 需要同时优化数据项与物理项损失？  
**4-3.** 正问题与反问题在工程应用中的目标有何不同？


### 基础题参考答案要点

**4-4.** PINN在损失函数中显式加入物理方程残差与边界/初值约束，区别于纯数据驱动网络。  
**4-5.** 数据项保证观测拟合，物理项保证机理一致性，两者共同决定工程可用性。  
**4-6.** 正问题是给定参数求状态演化，反问题是利用观测反演参数或边界条件。

### 应用题

**4-7.** 设计一个用于渠段水位预测的 PINN 训练流程。  
**4-8.** 设计“PINN参数更新失败”的回退与复核机制。

### 思考题

**4-9.** 在观测数据质量较差时，PINN 应更依赖物理项还是数据项？如何权衡？
