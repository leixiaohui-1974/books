...
1. 分别定义可控性与可观性。  
2. 可控性矩阵与可观性矩阵分别反映什么问题？  
3. 为什么“可观但不可控”仍是高风险状态？

> **术语一致性声明**：本章统一采用“运行设计域（Operational Design Domain, ODD）”“安全包络（Safety Envelope）”“分层分布式控制（Hierarchical Distributed Control, HDC）”等规范术语，与前序章节保持一致。

### 基础题参考答案要点

1. 可控性是输入对状态可达能力；可观性是输出对状态可重建能力。  
2. 可控性矩阵评估执行链路覆盖，\(\mathcal{O}\) 评估测量链路覆盖。  
3. 因无法有效施加调节动作，异常只能被观察而难以抑制。

### 应用题

4. 设计一个四节点系统的测点/执行器最小布设方案并说明理由。  
5. 某系统在丰水期可观、枯水期不可观，提出ODD收缩策略。

### 思考题

6. 在预算受限场景下，你会如何平衡“新增测点”与“新增执行器”的投资优先级？
