...
## 安全强化学习：ODD 与 Safety Envelope 约束
...
## SCADA+MAS Fusion Architecture 中的部署范式
...
本章给出强化学习在水系统中的工程化方法：以 MDP 统一问题定义，以 Q-learning/DQN/PPO/SAC 形成算法工具箱，以 ODD 与安全包络（Safety Envelope）构建安全约束。关键结论是：强化学习的价值不在“替代一切”，而在“在可控风险内提升复杂场景决策质量”。下一章将进入大语言模型与认知AI，讨论如何把专家知识、文档语义与实时运行状态整合为可交互的调度认知能力。

## 习题

### 基础题

**5-1.** 为什么水系统强化学习必须先完成 MDP 的工程语义建模？  
**5-2.** DQN 与 PPO 在动作空间假设上有何差异？  
**5-3.** 什么情况下应触发最小风险状态（Minimum Risk State）？

### 基础题参考答案要点

**5-4.** 因为状态、动作、奖励的定义直接决定策略学习方向；若工程语义缺失，策略会优化错误目标。  
**5-5.** DQN主要面向离散动作并通过Q值选最大动作；PPO可处理连续动作并通过策略梯度直接更新策略。  
**5-6.** 当策略不确定性超阈值、检测到ODD外工况、或动作可能触发Safety Envelope越界时应立即触发。

### 应用题

**5-7.** 设计一个泵站群协同控制的奖励函数，并说明每一项工程含义。  
**5-8.** 给出“离线训练—影子运行—分级放权”的门禁指标建议。

### 思考题

**5-9.** 当 RL 策略与值班工程师经验冲突时，系统应如何实现人机共融决策？请提出可执行流程。
