...
1. 为什么 `ctrl.command.*` 建议采用 QoS-2？
2. `idempotency_key` 在事件系统中解决了什么问题？
3. 解释一致性门禁公式中三个时延项。

### 应用题

4. 为一个“暴雨+多站联调”场景设计主题优先级与限流策略。  
5. 设计一份“事件重放审批单”，包含触发条件、环境限制、回放范围和审计字段。

### 思考题

6. 当“实时控制低时延”与“全量审计高可靠”冲突时，你会如何在架构上做取舍？

### 基础题参考答案要点

1. 因关键控制命令不能重复也不能丢失，需要最严格传输保证。  
2. 防止重复消息导致重复执行，保障事件处理幂等性。  
3. 分别对应时钟偏差、队列延迟、处理延迟，总和应小于门限。
