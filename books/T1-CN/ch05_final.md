...
> 
> **与后章的关系**：本章的非线性处理、不确定性量化为第七章"分层分布式控制"（原理三）和第九章"安全包络"提供建模基础
> 
> **核心概念**（6 个）：
...
ID/IDZ 模型建立在线性化假设之上，当水力工况偏离额定运行点时，非线性特性不可忽略。雷晓辉等从学科演进角度指出，水资源系统分析正经历从静态平衡到动态控制的范式转变 [5-R1]，这一转变对建模提出了新要求——传统基于稳态假设的线性模型难以适应实时运行控制场景中的多工况切换和动态响应需求。本节介绍工程中处理非线性问题的四种常用策略，从简单到复杂依次递进。
...
含时延的精确离散化采用 **Pade 近似**处理指数项 $e^{-\tau_i s}$：一阶 Pade 近似为 $e^{-\tau s} \approx (1 - \tau s/2)/(1 + \tau s/2)$，二阶近似为 $(1 - \tau s/2 + \tau^2 s^2/12)/(1 + \tau s/2 + \tau^2 s^2/12)$，在 $|\tau s|  \lt  1$ 范围内误差小于 2%。
...
当工况条件（如流量、水深、季节）使得模型不确定性 $\Delta\theta$ 超过鲁棒性设计所能容忍的最大值 $\Delta\theta_{max}$ 时，系统已经超出 ODD，应触发 MRC（最小风险条件）切换。这将在第九章安全包络框架中进一步定义和实现。
...
本章与第四章共同构成"水系统形式化建模"的完整知识体系。第四章提供基础工具（状态空间、传递函数、ID/IDZ），本章应对复杂场景（非线性、耦合、不确定性）。后续章节将在此基础上展开控制设计（第七章）、安全边界（第九章）和在环验证（第九章）。

---

## 附录 5-A 与在环验证和工程案例的联系

本章的高级建模技术在第九章"在环验证（xIL）"中得到全面应用：
...
[5-R1] 雷晓辉,许慧敏,何中政,等.水资源系统分析学科展望：从静态平衡到动态控制[J].南水北调与水利科技(中英文),2025,23(04):770-777.DOI:10.13476/j.cnki.nsbdqk.2025.0078.
