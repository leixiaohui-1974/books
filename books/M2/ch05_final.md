...
**5-1.** 分布式MPC与分散式控制的根本差异是什么？
**5-2.** 为什么 DMPC 必须显式处理边界一致性约束？
**5-3.** 同步式与异步式协调各自的主要优缺点是什么？

### 应用题

**5-4.** 针对三子网耦合场景，设计一套边界变量交换与协调迭代流程。  
**5-5.** 设计通信中断情况下的“局部自治—全局恢复”状态机，并给出切换阈值。

### 思考题

**5-6.** 若项目团队只能接受有限通信开销，你会优先牺牲最优性、鲁棒性还是响应速度？请给出理由。

### 基础题参考答案要点

**5-7.** DMPC存在显式协同机制与一致性目标，分散式通常缺少系统级协调。  
**5-8.** 因子网间共享边界变量，若不一致将导致全局物理不可行或性能劣化。  
**5-9.** 同步式一致性好但时延敏感；异步式实时性强但收敛与稳定性分析更复杂。
