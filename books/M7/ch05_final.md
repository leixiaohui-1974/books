...
**5-1.** 为什么任务编排必须与 ODD 和安全门禁联动？
**5-2.** SLA-A 与 SLA-C 的本质差异是什么？
**5-3.** 解释调度时序预算公式中四项时延含义。

### 应用题

**5-4.** 为某泵站群设计一份任务分级清单，包含任务类型、SLA等级、失败回退策略。  
**5-5.** 设计一个冲突仲裁评分表，给出权重设置依据与审批流程。

### 思考题

**5-6.** 当经济优化任务长期被安全任务压制时，你如何在不破坏安全底线前提下改进调度策略？

### 基础题参考答案要点

**5-7.** 因任务是否可执行取决于运行场景与安全约束，必须联动校验。  
**5-8.** SLA-A面向关键控制低时延高可靠，SLA-C面向非关键任务可延后。  
**5-9.** 分别是准入、仲裁、执行、提交时延，总和受控制周期约束。
