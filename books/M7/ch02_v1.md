...
1. 为什么组件职责边界不清会放大运行风险？
2. GIC 与 SSE 在控制链路中分别承担什么角色？
3. 解释架构级时序门禁公式中四个时延项的工程含义。

### 应用题

4. 为某泵站群设计一份组件依赖矩阵，标注强依赖与可降级依赖。  
5. 设计一份接口契约最小字段模板，并说明每个字段用于哪个门禁环节。

### 思考题

6. 如果新增一个认知AI组件，如何在不破坏既有实时性门禁的前提下接入 HydroOS？

### 基础题参考答案要点

1. 会导致故障定位困难、责任不清和连锁失效。  
2. SSE负责策略生成与编排，GIC负责安全校验与放行/拒绝。  
3. 分别对应接入、规划、门禁、执行时延，总和不得超过控制周期。
