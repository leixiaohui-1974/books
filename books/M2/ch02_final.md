...
**2-1.** 解释递归可行性与闭环稳定性的区别与联系。
**2-2.** 为什么MPC只执行首个控制量而不一次性执行整段序列？
**2-3.** 终端约束与软约束在工程上分别解决什么问题？

### 应用题

**2-4.** 给定线性渠段模型和约束区间，构建一组可用于求解器输入的MPC标准矩阵。  
**2-5.** 设计一套“无解触发—降级回退—恢复上线”的状态机逻辑，并说明门禁条件。

### 思考题

**2-6.** 在“保守性高但安全余量大”和“性能高但风险更敏感”之间，你将如何制定项目阶段性参数策略？

### 基础题参考答案要点

**2-7.** 递归可行性关注“每步都有解”，稳定性关注“闭环长期受控/收敛”；两者共同决定可上线性。  
**2-8.** 为利用新观测持续纠偏、抑制模型失配误差累积。  
**2-9.** 终端约束保障长期收敛结构，软约束保障突发工况下问题仍可解。
