...
> **术语一致性声明**：本章统一采用“分层分布式控制（Hierarchical Distributed Control, HDC）”“模型预测控制（Model Predictive Control, MPC）”“安全包络（Safety Envelope）”“运行设计域（Operational Design Domain, ODD）”“SCADA+MAS Fusion Architecture”等规范术语，与前序章节保持一致。

### 基础题参考答案要点

1. 因大规模系统存在通信、算力与组织边界约束，集中式方案易出现实时性与鲁棒性不足。  
2. L0负责秒级执行与联锁，L1负责分钟级局部优化，L2负责小时级全局协调与资源分配。  
3. 约束回传用于将局部可行域和风险信息反馈给上层，保证全局调度可执行且不越界。
