...
1. 随机切分会导致未来信息泄漏到训练集，造成评估结果虚高并误导上线决策。  
2. SVM适合小样本高维任务，RF鲁棒抗噪，GBDT在结构化特征上通常表现更优。  
3. 仅有精度不代表可安全运行，治理门禁（版本、审计、回退）决定工程可控性。

> **术语一致性声明**：本章统一采用“运行设计域（Operational Design Domain, ODD）”“安全包络（Safety Envelope）”“在环测试（In-the-Loop Testing）”“多智能体系统（Multi-Agent System, MAS）”“SCADA+MAS Fusion Architecture”“模型预测控制（Model Predictive Control, MPC）”等规范术语，并与前序章节保持一致。
