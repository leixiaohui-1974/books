...
**6-1.** 为什么算法服务托管化是 HydroOS 的必要条件？
**6-2.** 资源隔离策略中“故障隔离”解决了什么问题？
**6-3.** 解释托管时序门禁公式各项时延的意义。

### 应用题

**6-4.** 为一个双机房部署场景设计算法服务资源配额方案，包含SLA-A/B/C三类服务。  
**6-5.** 设计一份算法灰度发布门禁表，至少包含时延、约束违背率、告警触发率三项指标。

### 思考题

**6-6.** 当新模型精度更高但时延略高时，你会如何设置发布准入与审批策略？

### 基础题参考答案要点

**6-7.** 托管化可保证环境一致、版本可控、故障可隔离与审计可追溯。  
**6-8.** 防止单个服务崩溃扩散到全局控制链路。  
**6-9.** 分别对应装载、推理、校验、提交时延，总和必须受控制周期约束。
