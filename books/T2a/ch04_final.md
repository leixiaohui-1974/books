...
> **术语一致性声明**：本章统一采用“降阶模型（Reduced-Order Model, ROM）”“模型预测控制（Model Predictive Control, MPC）”“运行设计域（Operational Design Domain, ODD）”“安全包络（Safety Envelope）”等规范术语，与前序章节保持一致。

### 基础题参考答案要点

1. ROM 是“保留关键动态与关键约束后的结构化简化”，不是去掉难点。  
2. IDZ: \(G(s)=K\frac{1+T_z s}{s}e^{-\tau s}\)，\(K\) 体现稳态增益，\(\tau\) 体现纯延时。  
3. Muskingum-Cunge 系数满足 \(C_0+C_1+C_2=1\)，用于保证质量平衡一致性。
