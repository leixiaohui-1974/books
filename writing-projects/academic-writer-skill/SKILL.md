...
步骤5.5: 【长文档策略选择——不可跳过】
        根据提纲估算目标字数，按§5.1分级策略选择生成方式：
        - ≤5000字 → L1单文件直出
        - 5001-12000字 → L2数据-引擎分离
        - 12001-25000字 → L3分段组装(Markdown→pandoc→docx)
        - >25000字 → L4多文件分册
        必须在回复中声明："预估X字，采用LN级策略"
步骤6: 参照 `references/writing_craft_guide.md` 对应文体技法，按文体模板和§5策略撰写初稿
...
> **核心原则：任何长度的文档都必须成功生成，绝不允许因内容过长而反复失败。**

### 5.0 问题诊断

在 claude.ai 网页端用计算机工具生成 .docx 文档时，实际执行路径是：
1. 用 `create_file` 写一个 .js 脚本，脚本内嵌全部中文内容作为字符串字面量
2. 用 `bash_tool` 执行 `node xxx.js` 生成 .docx

**失败根因**：当文档内容超过约 8000 字（约 5000 行 JS 代码）时：
- `create_file` 工具调用传输的数据量过大，导致超时或截断
- 单个 JS 文件中字符串字面量过多，Claude 输出被截断
- 即使成功创建文件，执行时也可能因字符串拼接过大而失败
- 反复重试只会反复失败，浪费对话轮次

### 5.1 强制分级策略

**在撰写任何文档初稿前，必须先估算目标字数，然后选择对应级别的生成策略。**

| 级别 | 目标字数 | 策略名称 | 核心方法 |
|------|---------|---------|---------|
| **L1** | ≤5000字 | 单文件直出 | JS 脚本直接内嵌全部内容 |
| **L2** | 5001-12000字 | 数据-引擎分离 | 内容写入 JSON 数据文件，JS 引擎读取数据生成 docx |
| **L3** | 12001-25000字 | 分段组装 | 内容按节写入多个 .md 文件，最后合并转换 |
| **L4** | >25000字 | 多文件分册 | 拆为多个独立 .docx，由用户合并 |

### 5.2 L1 单文件直出（≤5000字）

适用于：短篇公众号文章、单篇专利摘要、短报告等。

直接在一个 JS 文件中用 `docx` 库内嵌全部内容生成，与当前做法相同。无需特殊处理。

### 5.3 L2 数据-引擎分离（5001-12000字）

适用于：标准论文、专利全文、短章节、中等报告等。

**核心思路**：把"内容数据"和"格式引擎"彻底分离为两个文件。

**步骤**：

```
步骤 A: 创建数据文件 content.json
       - 将全文按段落/节拆为数组
       - 每个元素标注类型(h1/h2/h3/para/table/formula)
       - 单个 create_file 调用只写纯文本数据，无 JS 代码开销

步骤 B: 创建引擎文件 engine.js (固定模板，约200行)
       - 读取 content.json
       - 按类型标记生成对应的 docx Paragraph/Table 对象
       - 引擎是通用的，不含任何具体内容

步骤 C: 执行 node engine.js → 输出 .docx
```
...
```json
{
  "meta": { "title": "章节标题", "author": "雷晓辉", "font_cn": "SimSun", "font_en": "Times New Roman" },
  "sections": [
    { "type": "h1", "text": "第一章 绪论" },
    { "type": "para", "text": "正文段落内容..." },
    { "type": "h2", "text": "1.1 研究背景" },
    { "type": "para", "text": "更多正文..." },
    { "type": "table", "headers": ["列1","列2"], "rows": [["数据1","数据2"]] },
    { "type": "formula", "text": "Q = A × v    ......(1)" },
    { "type": "pagebreak" }
  ]
}
```

**如果 content.json 仍然过大**（超过约 4000 行），则拆为多个数据文件：
```
content_part1.json  (前半部分)
content_part2.json  (后半部分)
engine.js           (依次读取两个文件，合并后生成)
```

**引擎模板要点**：
```javascript
// engine.js 核心结构（仅示意，不含具体内容）
const data = JSON.parse(fs.readFileSync("content.json", "utf-8"));
const children = data.sections.map(item => {
  switch(item.type) {
    case "h1": return new Paragraph({ heading: HeadingLevel.HEADING_1, children: [new TextRun({text: item.text, ...})] });
    case "h2": return new Paragraph({ heading: HeadingLevel.HEADING_2, ... });
    case "para": return new Paragraph({ spacing: {...}, children: [new TextRun({text: item.text, ...})] });
    case "table": return buildTable(item.headers, item.rows);
...
### 5.4 L3 分段组装（12001-25000字）

适用于：书稿章节（完整一章）、长报告、行业标准全文等。

**核心思路**：先用 Markdown 分段写内容，最后用 pandoc 一步转 docx。

**步骤**：

```
步骤 A: 规划结构——确定全文分为几个片段(part)，每个片段 ≤4000字

步骤 B: 逐片段创建 Markdown 文件
       create_file → part01.md  (第一节内容)
       create_file → part02.md  (第二节内容)
       create_file → part03.md  (第三节内容)
       ...每个文件独立调用，确保每次都不超限

步骤 C: 合并所有片段
       bash: cat part01.md part02.md part03.md > full_document.md

步骤 D: 用 pandoc 转换为 docx（可指定参考模板）
       bash: pandoc full_document.md -o document.docx \
             --reference-doc=template.docx  (可选，控制样式)

步骤 E: 如需精细格式控制（表格样式、页眉页脚等），
       用 unpack → 编辑 XML → repack 方式微调
```

**Markdown 片段的写作要求**：
- 每个片段文件以 `<!-- PART N: 节标题 -->` 开头，方便定位
- 片段之间的标题层级必须连续，不可跳级
- 表格使用 Markdown 标准语法（pandoc 可正确转换）
- 公式使用 LaTeX 语法 `$...$` 或 `$$...$$`
- 图表占位用 `[图X: 描述]` 标记
...
# 带样式模板转换（推荐）
pandoc full.md -o output.docx --reference-doc=template.docx

# 带目录
pandoc full.md -o output.docx --toc --toc-depth=3
...
**策略**：按章/节拆为多个独立 .docx 文件，每个文件用 L2 或 L3 策略生成。

```
步骤 A: 规划全文结构，按自然章节边界拆分
       第一章.docx  (用 L3 策略)
       第二章.docx  (用 L3 策略)
       ...

步骤 B: 每个分册独立完成 写-审-改 闭环

步骤 C: 输出时明确告知用户：
       "已生成N个分册文件，请在Word中合并为完整文档"
       附合并操作指引
```

### 5.6 关键防失败规则（⚠️ 必须遵守）

1. **禁止盲目重试**：如果一次 `create_file` 失败或被截断，**禁止用相同策略重试**。必须立即升级到更高级别策略（L1→L2→L3→L4）。

2. **禁止单个 `create_file` 超过 800 行**：无论任何文件（JS脚本、JSON数据、Markdown），单次 `create_file` 调用的内容不得超过约 800 行。超过时必须拆分为多次调用。

3. **JSON 数据文件拆分规则**：当 content.json 需要超过 600 行时，按节/章拆为 `content_part1.json`、`content_part2.json` 等。引擎文件依次读取合并。

4. **Markdown 片段拆分规则**：每个 .md 片段文件不超过 4000 字（约 150-200 行），按节标题边界拆分。

5. **预估先行**：在开始任何写作之前，必须先根据提纲估算总字数，然后在回复中明确声明："预估 X 字，采用 LN 级策略"。

6. **渐进式构建**：对于 L2/L3 策略，每创建一个文件后用 `bash` 验证文件完整性（如 `wc -l`, `wc -c`, `python -m json.tool` 等），确认无截断后再创建下一个文件。

7. **引擎文件复用**：L2 的 engine.js 是通用模板，一旦成功创建可跨文档复用（仅需替换数据文件）。建议在项目中保留一份标准引擎文件 `templates/docx_engine.js`。

### 5.7 各文体的典型级别

| 文体 | 典型字数 | 推荐级别 | 说明 |
|------|---------|---------|------|
| WX | 1500-5000字 | L1-L2 | 短文L1，长文L2 |
| PAT | 4000-8000字 | L2 | 含权利要求书通常超5000字 |
| SCI | 6000-12000字 | L2 | 大多数论文适用 |
| CN | 5000-10000字 | L2 | 同上 |
| RPT | 8000-30000字 | L2-L3 | 视报告长度选择 |
| STD-CN | 8000-20000字 | L2-L3 | 行标通常需要L3 |
| STD-INT | 8000-20000字 | L2-L3 | 同上 |
| BK(单章) | 8000-20000字 | L2-L3 | 按章生成，每章独立 |
| BK(全书) | 80000-200000字 | L4 | 必须分册 |
| PPT | N/A | 专用 | 使用 pptxgenjs，不受此限制 |
...
| 故障类型 | 识别标志 | 恢复动作 |
|---------|---------|---------|
| `create_file` 超时/截断 | 文件内容不完整或工具报错 | 立即升级策略等级，拆分后重新生成 |
| `node xxx.js` 执行失败 | JavaScript 报错 | 检查 JSON 数据文件格式，修复后重试；若 JS 本身太大则升级到 L3 |
| pandoc 转换失败 | pandoc 报错 | 检查 Markdown 语法，修复后重试 |
| 文件丢失/覆盖 | `ls` 验证发现文件不存在 | 从最近成功的步骤重新执行 |
| 连续 2 次同级别失败 | 两次相同策略都失败 | **强制升级到下一级别**，不再在当前级别重试 |
...
| CONS-10 | §6.1术语表中英对照 | 所有文体（WX用§6.3翻译表，PPT遵循同一术语体系） |
...
| **create_file超时/截断** | **禁止同级重试，立即升级策略等级（§5.1），拆分后重新生成** |
| **JS脚本执行内存溢出** | **升级到L3(Markdown+pandoc)策略，避免JS内嵌大量字符串** |
| **连续2次生成失败** | **强制升级到下一级策略（L1→L2→L3→L4），不再在当前级别尝试** |
| **docx验证失败** | **检查生成的XML，用validate.py定位问题，修复后repack** |
