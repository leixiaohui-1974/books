...
When predicted trajectories exit \(\mathcal{X}_{safe}\), systems should trigger graded mitigation: policy clipping, mode degradation, or human takeover. This coupling is central to trustworthy deployment.
...
## 2.6 Why CHS is distinct from “AI-only” approaches

AI-only operation often fails at deployment boundaries because it under-specifies hard constraints, responsibility, and lifecycle governance. CHS explicitly couples:
...
This integrated framing is the practical reason CHS is suitable for critical infrastructure.

## 2.7 Chapter summary and bridge to Chapter 3

This chapter formalized the theoretical backbone of CHS through governing equations, control-oriented abstraction, the Eight Principles, and ODD–Safety Envelope coupling. The next step is architecture: Chapter 3 translates these foundations into implementable system design, including WNAL L0-L5, HydroOS three-tier structure, and SCADA+MAS Fusion Architecture.
