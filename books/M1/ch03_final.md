<!-- 变更日志
v2 2026-02-16: 根据评审补充基础题答案要点与质量检查清单
v1 2026-02-16: 初稿
-->
...
1. $K$ 是静态增益，反映稳态灵敏度；$T$ 是主导动态时间尺度；$\tau$ 是传播与执行延迟合成。  
2. 因为参数少、辨识稳定、可解释性强，能在工程约束下快速上线并支持回退。  
3. 当出现明显超调、双时标响应或高频拟合误差持续偏大时，应考虑 SOPDT。

> **术语一致性声明**：本章统一采用“传递函数（Transfer Function）”“降阶模型（Reduced-Order Model, ROM）”“运行设计域（Operational Design Domain, ODD）”“安全包络（Safety Envelope）”“模型预测控制（Model Predictive Control, MPC）”“分层分布式控制（Hierarchical Distributed Control, HDC）”“在环测试（In-the-Loop Testing）”等规范术语。

### 质量检查清单（执行结果）

- [x] 学习目标、例题、小结、习题、拓展阅读齐备；  
- [x] 基础题≥3、应用题≥2、思考题≥1，且含基础题答案要点；  
- [x] 图表占位≥2（图3-1、图3-2、表3-1）；  
- [x] 公式均配“物理直觉/工程解释”；  
- [x] 章首回顾上一章结论，章末预告下一章 IDZ。
