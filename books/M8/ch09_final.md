...
**9-1.** 为什么硬约束应作为可行域约束而不是软惩罚项？
**9-2.** 写出库容连续方程的离散形式，并解释各项物理意义。
**9-3.** 说明 MRS 中松弛变量与常规软约束惩罚的区别。

### 应用题

**9-4.** 依据本章框架，构建一个 6 步滚动优化的变量与约束清单。  
**9-5.** 设计一条“不可行检测→MRS触发→治理留痕→退出MRS”的状态机。

### 思考题

**9-6.** 若长期频繁触发 MRS，应优先调整模型、设备、还是调度规则？请给出分层治理建议。

### 基础题参考答案要点

**9-7.** 因硬约束对应安全与结构边界，违反代价不可被经济收益抵消。  
**9-8.** $s_{k+1}=s_k+\Delta t(Q_{in,k}-Q_{out,k})$，表示库容由净入流累积变化。  
**9-9.** MRS 松弛变量用于“不可行时最小违约”，常规软约束用于“可行域内优化偏好”。
