<!-- 变更日志
v3 2026-03-03: 三角色评审修订——(1)§7.7.2 "Agent"术语替换为物理控制层术语；(2)§7.7.3新增式(7-39b) DMPC本地代价函数展开形式；(3)§7.9新增"数字安全包络"小节，包含MPC控制律李普希茨约束实现（Δu指令限速）；(4)§7.9新增Water-CIM接口设计说明。
...
[图7-1: MPC在CHS四层架构中的位置]
{描述: 四层垂直堆叠。L0安全层（灰色）在底部，L1调节层（浅蓝）包含多个PI回路，L2协调层（深蓝）包含MPC优化器并向L1下发设定值，L3计划层（紫色）在顶部。MPC与安全包络之间有红色约束注入箭头。标注采样周期和通信周期。}
{尺寸: 半页}
{颜色方案: 蓝色系}
{对应ARCH编号: ARCH-04}

MPC与安全包络的接口也在L2层实现：安全包络定义的可行状态集$\mathcal{X}_{\text{safe}} \subseteq \mathcal{X}_{\text{ODD}}$直接作为MPC的状态约束注入优化问题。这确保了MPC的最优解不仅满足性能目标，还始终位于安全包络之内（Lei 2025d）。
...
[图7-2: 三渠池$\boldsymbol{\Theta}$矩阵稀疏结构示意]
{描述: 45×15矩阵的热力图，非零元素用深蓝色标记，零元素为白色。呈现明显的阶梯状稀疏模式：左上角大片白色（延迟导致的零区），下三角区域有条带状非零区。标注各渠池的延迟导致的零行数。}
{尺寸: 半页}
{颜色方案: 蓝色系}
...
**结果讨论**：本例展示了MPC整定中最常见的取舍——跟踪精度与执行器寿命。Bryson规则提供了定量的起点，但最终的整定需要综合考虑运行经验和设备状况。这一迭代过程通常在MPC上线前的MIL（模型在环）测试阶段完成(Lei 2025b)。
...
[图7-2a: 线性MPC vs NMPC选型决策树]
{描述: 从上到下的决策树。根节点："系统非线性程度如何？"→"线性模型预测误差<控制容忍偏差的50%？"→是：选择线性MPC→否："是否仅在少数极端工况下非线性显著？"→是：线性MPC+增益调度→否："计算预算是否允许在线NLP求解？"→是：NMPC(RTI)→否：线性MPC+离线NMPC校核。每个叶节点标注适用条件和风险提示。}
{尺寸: 半页}
{颜色方案: 蓝色系}
...
- **L2层（区域MPC子系统）**：每个管理区段运行一个区域MPC，以本区段的渠池为对象，优化本区段的控制策略。各区域MPC子系统之间通过边界流量一致性约束进行协调；
- **L3层（全局协调器）**：全局协调层，负责在各区域MPC子系统之间分配总流量目标、协调边界条件、处理全局约束（如总供水量守恒、全局能耗最小化）。

$$
\text{HDMPC} = \{\text{L1边缘控制器}\} + \{\text{L2区域MPC子系统} \times M\} + \{\text{L3全局协调器}\} \tag{7-39}
$$

[工程解释] HDMPC的核心优势是**分层解耦**：L2层的各区域MPC子系统问题规模小（通常3~10个渠池），QP求解时间极短；L3层的协调问题只涉及区域间的边界变量，维度远小于全局问题。同时，HDMPC自然匹配水网的管理架构——每个区域MPC子系统对应一个管理单位，具有本地自治权(Lei 2025a)。

[图7-3: 集中式与分布式MPC架构对比]
{描述: 左侧集中式——单一大型优化器接收所有传感器数据并下发所有控制指令，中心节点为瓶颈。右侧分布式——多个区域MPC并行运行，通过协调层交换边界信息。底部标注计算量、通信量和管理兼容性对比。}
{尺寸: 全页}
{颜色方案: 蓝色系}
{对应ARCH编号: ARCH-04}

**【式7-39b: DMPC本地代价函数标准展开形式（与§O规范一致）】**

$$
J_i = \sum_{k=0}^{N_p-1} \left[ \| y_i(k) - r_i(k) \|^2_{Q_i} + \| \Delta u_i(k) \|^2_{R_i} \right] \tag{7-39b}
$$

其中 $y_i(k)$ 为第 $i$ 个子系统在预测步 $k$ 的输出（水位偏差，m），$r_i(k)$ 为参考轨迹，$Q_i \succeq 0$ 为子系统跟踪权重矩阵，$\Delta u_i(k)$ 为控制增量（闸门开度变化量），$R_i \succ 0$ 为子系统控制权重矩阵，$N_p$ 为预测时域。式(7-39b)与CLAUDE.md §O规范中DMPC代价函数完全一致，是全局目标式(7-40)中 $J_i(\Delta\mathbf{U}_i)$ 的显式展开形式。各子系统在不交换本地代价函数参数的前提下独立求解式(7-39b)，协调信息仅通过边界耦合变量传递（见§7.7.3）。
...
其中$J_i$为第$i$个子系统的本地代价，$\mathbf{A}_{\text{couple}}$和$\mathbf{b}_{\text{couple}}$为子系统间的耦合约束（边界流量一致性）。
...
**岛屿自治模式（Island Autonomy）。** 当某个区域Agent与协调层或相邻区域的通信中断时，该区域自动进入岛屿自治模式：
...
**$\mathcal{X}_{\text{safe}} \subseteq \mathcal{X}_{\text{ODD}}$约束自动注入。** 在CHS框架中，安全包络定义了运行设计域（ODD）内的安全状态集$\mathcal{X}_{\text{safe}}$。MPC的状态约束不应由控制工程师手动设定，而应从安全包络数据库中自动提取并注入优化问题(Lei 2025d)。自动注入的好处是：（1）避免人工设定约束时的遗漏或错误；（2）当安全包络因工况变化而动态调整时（如汛期收缩ODD），MPC的约束自动同步更新。
...
[图7-4: MPC约束与安全包络三区间的对应关系]
{描述: 一维水位轴，从下到上：红区下界→MPC下限（含裕度）→黄区下界→绿区（正常运行）→黄区上界→MPC上限（含裕度）→红区上界。标注MPC可行域、安全裕度和三区间边界。}
{尺寸: 半页}
{颜色方案: 红/黄/绿三色}
{对应ARCH编号: ARCH-07}
...
### 7.9.6 数字安全包络：MPC控制增量的李普希茨约束实现

**问题动因：水锤与控制增量速率限制。** 在明渠和有压管道水网中，闸门/泵站的快速动作会激发水锤波（水击现象）。水锤压力波的峰值可比正常运行压力高出数倍，是导致管道爆裂、闸门损毁的主要原因之一。常规MPC的控制增量约束 $|\Delta u_k| \leq \Delta u_{\max}$ 仅限制单步最大变化幅度，但并未从连续动力学角度保证控制轨迹的光滑性。**数字安全包络**（Digital Safety Envelope, DSE）在MPC求解完成后、指令下发前，增加一层李普希茨连续性滤波，确保相邻控制指令之间的变化率始终在物理安全范围内。

**李普希茨约束的形式化。** 设第 $i$ 个执行器（闸门或泵）的连续控制信号为 $u_i(t)$，其控制率被约束满足：

$$
\left| \frac{du_i}{dt} \right| \leq L_i \tag{7-49}
$$

其中 $L_i$ 为执行器 $i$ 的李普希茨常数（单位：%/s 或 rpm/s），由执行器物理机械特性和水力安全分析共同确定。在离散采样域，式(7-49)等价于：

$$
|\Delta u_{i,k}| = |u_{i,k} - u_{i,k-1}| \leq L_i \cdot \Delta t \triangleq \Delta u_{i,\max} \tag{7-50}
$$

其中 $\Delta t$ 为MPC控制采样周期。$\Delta u_{i,\max}$ 即为REVIEW_REPORT.md中记录的控制增量上界约束，已纳入标准QP的约束矩阵（式7-20）。

**数字安全包络算法（DSE后处理层）。** DSE作为MPC输出与执行器之间的"安全门"实现如下：

```python
def digital_safety_envelope(u_cmd, u_prev, delta_u_max, u_min, u_max):
    """
    MPC控制指令的数字安全包络后处理。

    参数:
        u_cmd     : MPC求解器输出的期望控制量 [开度 %]
        u_prev    : 上一时刻实际执行的控制量 [开度 %]
        delta_u_max: 最大允许单步增量 [%/step] = L_i * Delta_t
        u_min, u_max: 执行器物理行程约束 [%]

    返回:
        u_safe   : 经DSE过滤后的安全控制指令
        clamped  : 布尔标志，True表示本步发生了限速截断
    """
    # 1. 计算期望增量
    delta_u = u_cmd - u_prev

    # 2. 李普希茨限速截断
    if abs(delta_u) > delta_u_max:
        delta_u_clipped = delta_u_max * (1 if delta_u > 0 else -1)
        clamped = True
    else:
        delta_u_clipped = delta_u
        clamped = False

    # 3. 物理行程约束
    u_safe = float(np.clip(u_prev + delta_u_clipped, u_min, u_max))

    return u_safe, clamped
```

**工程实施要点：**

1. **$\Delta u_{\max}$ 的整定。** 闸门执行机构的典型值为 $\Delta u_{\max} = 2\%$~$5\%$ 开度/采样步（$\Delta t = 5$~$15$ min）。对于有压管道泵站，水锤分析通常要求启泵时间不少于30~60秒，对应 $L_{\text{pump}} \leq 3.3\%$/s。整定时优先参考水力过渡过程计算报告（由水工设计单位提供）。

2. **DSE触发审计。** 每次 `clamped=True` 应写入运行日志，记录时间戳、执行器编号、期望增量和实际增量。连续多步 `clamped=True` 表明MPC优化过于激进——此时应检查MPC中 $\mathbf{R}$ 矩阵是否偏小，或加大QP约束中的 $\Delta u_{\max}$ 参数与DSE的 $\Delta u_{\max}$ 保持一致（两者应相等）。

3. **与ch10安全包络的关系。** DSE是ch10"安全包络"框架（§10.3执行器约束层）在MPC输出端的具体实现。DSE负责控制增量速率约束（李普希茨层），ch10的安全联锁（PLC层）负责绝对量约束（硬约束层）。两者互补，DSE在上层、联锁在底层，形成"软限速+硬截断"双重保障（参见ch10 §10.4.2）。

[工程解释] 在胶东调水工程的实际部署经验中，闸门动作过快曾导致渠道水位振荡（量级约±15cm，超出±5cm的正常运行带），原因是MPC优化给出的控制增量（12%开度/步）远超执行机构的安全动作速率（5%/步）。增加DSE后处理层后，水位振荡消除。这验证了李普希茨约束实现对控制品质的实际意义。

### 7.9.7 Water-CIM接口设计

**背景：物理AI算法的语义对象边界。** 随着水网数字化深入，MPC算法不再孤立运行，而是嵌入在更大的Water-CIM（Water Common Information Model，水网通用信息模型）语义架构中。Water-CIM将水网中的物理对象（渠段、泵站、闸门、水库）和控制对象（MPC控制器、PI调节器、预报模型）统一描述为具有标准化属性和接口的**语义对象**，使来自不同厂商的系统能够互操作。MPC算法在Water-CIM语义框架中的接口设计，决定了它能否被上层调度系统（如T2b描述的认知AI引擎）正确调用和监控。

**MPC的Water-CIM输入接口。** MPC控制器从Water-CIM数据总线消费以下语义对象：

| Water-CIM语义对象 | 对应MPC物理含义 | 数据类型 | 刷新周期 |
|-----------------|---------------|---------|---------|
| `CanalReach.waterLevel` | 渠池当前水位 $y_k$ [m] | Float64 | $\Delta t$（实时） |
| `CanalReach.flowRate` | 渠池当前流量 $Q_k$ [m³/s] | Float64 | $\Delta t$（实时） |
| `TargetSetpoint.levelRef` | 水位参考轨迹 $r_k$ [m] | Float64[] | 调度周期（15min~1h） |
| `DemandForecast.withdrawalQ` | 下游需水预报 $d_k$ [m³/s] | Float64[] | 预报更新周期 |
| `SafetyEnvelope.levelBounds` | 水位安全边界 $[y_{\min}, y_{\max}]$ [m] | Float64[2] | 事件驱动 |
| `ActuatorStatus.openingPct` | 执行器当前开度 $u_{k-1}$ [%] | Float64 | $\Delta t$（实时） |

**MPC的Water-CIM输出接口。** MPC控制器向Water-CIM数据总线发布以下语义对象：

| Water-CIM语义对象 | 对应MPC物理含义 | 数据类型 | 更新周期 |
|-----------------|---------------|---------|---------|
| `GateCommand.openingPct` | MPC控制指令 $u_k^*$ [%] | Float64 | $\Delta t$（每步） |
| `MPCState.predLevel` | 预测水位轨迹 $\hat{y}_{k:k+N_p}$ [m] | Float64[] | $\Delta t$ |
| `MPCState.costValue` | 当前步代价 $J_k^*$ | Float64 | $\Delta t$ |
| `MPCState.constraintSlack` | 约束裕度向量 $\boldsymbol{\varepsilon}_k$ | Float64[] | $\Delta t$ |
| `MPCState.solveTime` | QP求解耗时 [ms] | Float64 | $\Delta t$ |
| `MPCState.status` | 求解状态（optimal/infeasible/timeout） | Enum | 事件驱动 |

**接口设计原则：**

1. **输入单向依赖原则。** MPC只从Water-CIM消费数据，不直接访问传感器硬件。传感器→SCADA→Water-CIM数据总线→MPC，保持层次隔离。

2. **输出完整性原则。** MPC除下发控制指令外，还必须发布预测轨迹和求解状态。上层认知AI引擎（T2b描述）依赖这些信息进行决策解释和异常检测。`MPCState.status = infeasible` 是触发人工干预流程的标准信号。

3. **安全约束来自Water-CIM原则。** MPC的安全边界约束 $[y_{\min}, y_{\max}]$ 从 `SafetyEnvelope.levelBounds` 读取，而不是硬编码在MPC参数文件中。这确保了安全包络更新（如汛期放宽蓄水上限）能够自动传递到MPC约束层，无需重新部署MPC软件（参见ch10 §10.5.1 安全包络动态更新机制）。

4. **版本兼容性原则。** Water-CIM接口应遵循语义版本控制。MPC算法升级时，只要接口语义对象的名称和数据类型不变，上层调度系统无需修改。

[工程解释] Water-CIM接口设计是物理AI算法（T2a）与认知AI引擎（T2b）之间的"合同"。T2b的MCP协议（T2b第三章）通过Water-CIM语义对象调用T2a的MPC算法——认知AI不需要知道MPC内部的QP矩阵，只需要按Water-CIM接口发布设定值、读取预测轨迹即可。这种解耦设计使物理AI和认知AI可以独立迭代升级。
...
**工程主线。** MPC的工程部署核心是与CHS框架的集成（§7.9）：与安全包络的约束接口、与L1层PI控制器的无扰切换、QP求解器的嵌入式部署、上线前的系统检查清单；以及两项关键工程扩展——**数字安全包络**（§7.9.6，李普希茨控制增量约束实现，防止水锤）和**Water-CIM接口设计**（§7.9.7，物理AI算法与上层语义框架的标准化对接）。
