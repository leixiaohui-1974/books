...
## 认知AI与 SCADA+MAS Fusion Architecture 融合
...
**6-1.** 为什么认知AI引擎必须要求“回答带出处”？  
**6-2.** RAG 与知识图谱在功能上有何互补关系？  
**6-3.** 为什么认知AI不应直接绕过执行层联锁下发控制指令？

### 基础题参考答案要点

**6-4.** 因为调度建议必须可审计、可复核；无出处的回答无法作为生产决策依据。  
**6-5.** RAG侧重从文档中检索证据，知识图谱侧重实体关系与规则一致性，两者组合可同时提升可追溯性与推理一致性。  
**6-6.** 执行层联锁是安全底线；认知AI存在不确定性，必须经过规则校验与人工确认后才能影响控制动作。

### 应用题

**6-7.** 设计一个“告警处置建议包”的标准输出模板（字段不少于6项）。  
**6-8.** 给出认知AI影子运行阶段的人机一致性评估指标。

### 思考题

**6-9.** 当规程条款与历史案例结论冲突时，认知AI应如何排序证据优先级？请给出可执行规则。
