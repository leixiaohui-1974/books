...
**5-1.** Muskingum 参数 $K$ 与 $X$ 各代表什么物理意义？  
**5-2.** 为什么 $X$ 通常取值在 $[0,0.5]$？  
**5-3.** 递推系数 $C_0,C_1,C_2$ 为什么应满足和为 1？

### 基础题参考答案要点

**5-4.** $K$ 反映波传播时间尺度，$X$ 反映平移与扩散权重分配。  
**5-5.** 该区间可保证模型具有合理扩散特性并减少非物理振荡风险。  
**5-6.** 系数和为1体现离散质量守恒一致性，避免系统性偏差累积。

### 应用题

**5-7.** 设 $K=5\,\text{h}, X=0.25, \Delta t=1\,\text{h}$，已知 $I_t=180, I_{t+1}=210, O_t=170\,\text{m}^3/\text{s}$，计算 $O_{t+1}$。  
**5-8.** 设计一个 Muskingum 参数率定方案，要求包含样本选择、目标函数、验证指标与上线门禁。

### 思考题

**5-9.** 在冰期或强人类调控（频繁闸控）条件下，Muskingum 参数可能时变。你会采用“分季节参数库”还是“在线更新参数”？请说明工程权衡。
