...
1. 解释 PI 与 PID 在水系统中的典型适用差异。  
2. 为什么执行器饱和时必须配置 anti-windup？  
3. 前馈-反馈组合相比纯反馈的主要收益是什么？

> **术语一致性声明**：本章统一采用“模型预测控制（Model Predictive Control, MPC）”“运行设计域（Operational Design Domain, ODD）”“安全包络（Safety Envelope）”“分层分布式控制（Hierarchical Distributed Control, HDC）”等规范术语，与前序章节保持一致。

### 基础题参考答案要点

1. PI 适合慢过程稳态控制，PID 适合对动态速度有更高要求且噪声可控场景。  
2. 执行器饱和会导致积分累积失真，恢复后易产生大超调与长时间振荡。  
3. 前馈可提前抵消可预测扰动，反馈负责纠偏，组合可降低峰值偏差与恢复时间。

### 应用题

4. 针对双泵站液位系统，给出一个最小可执行的前馈-反馈控制框架并列出关键约束。  
5. 某回路相位裕度偏低，请提出两种改进策略并分析副作用。

### 思考题

6. 在“预测质量波动频繁”的场景中，如何设计前馈权重的自适应调节逻辑，避免频繁误切换？
