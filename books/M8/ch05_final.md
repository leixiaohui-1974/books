...
**5-1.** 为什么胶东调水场景特别适合采用 MPC？
**5-2.** 解释目标函数中三项权重对应的工程含义。
**5-3.** 求解超时时为何优先采用“上周期可行解”而非停控？

### 应用题

**5-4.** 针对一个长时滞渠段，给出 $T_p,T_c$ 的整定思路与验证方法。  
**5-5.** 设计一个 MPC 回退分级策略表，至少包含 3 级触发条件与动作。

### 思考题

**5-6.** 当供水目标与安全约束持续冲突时，MPC 权重应如何动态调整，谁来批准该调整？

### 基础题参考答案要点

**5-7.** 因其多约束、长时滞、扰动强且需滚动决策。  
**5-8.** 分别对应跟踪性能、动作平顺与安全惩罚。  
**5-9.** 可保持控制连续性并降低突发风险。
