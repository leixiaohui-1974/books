...
1. 为什么多层级协同调度需要不同时间尺度的决策层？
2. 说明一致性残差 $\varepsilon^{(r)}$ 在工程中的作用。
3. 冻结窗口为什么要分为前、中、后三个时段？

### 应用题

4. 为一个“上游水库—中游泵站—下游闸站”系统设计 L2/L1/L0 的输入输出接口。  
5. 设计你所在单位的交接班冻结窗口参数（$T_f^{pre},T_f^{handover},T_f^{post}$）并说明依据。

### 思考题

6. 当分布式协调在时限内未收敛，但业务目标必须按时达成时，你会如何平衡安全、效率与治理三目标？

### 基础题参考答案要点

1. 因为目标分配、局部协调和设备执行的时标不同，统一时标会导致计算与执行失配。  
2. 用于衡量各子系统是否达到可执行一致，是触发降级或仲裁的重要依据。  
3. 三时段设计可覆盖“预防扰动—责任交接—平稳恢复”完整过程。
...
> **术语一致性声明**：本章统一采用“分层分布式控制（Hierarchical Distributed Control, HDC）”“运行设计域（Operational Design Domain, ODD）”“安全包络（Safety Envelope）”“最小风险状态（Minimal Risk State, MRS）”“模型预测控制（Model Predictive Control, MPC）”等规范术语；数学符号采用 $Q,h,\tau,T_p,T_c,\mathbf{x},\mathbf{u},\mathbf{y},J$。
...
- [x] 公式前有物理直觉，公式后有工程解释；
- [x] 含可复现实例（例5-1）；
- [x] 图表不少于2项（图5-1、图5-2、表5-1、表5-2）；
- [x] 章首回顾ch04，章末预告ch06；
- [x] 已补齐ch04评审提出的协同时序图、收敛性直觉说明与冻结窗口模板。
