...
本章处理第四章基础建模方法无法覆盖的复杂工程问题：非线性特性、系统耦合、参数漂移、不确定性等。第四章建立了线性化的基础模型框架，本章则探讨"当模型不够精确、工况偏离额定运行点、系统参数随时间漂移"时如何处理——这些是真实水利工程中的常态问题。

本章介绍了工程上处理这些问题的五大类技术：增益调度和反馈线性化（§5.1），通过局部线性化或精确线性化处理非线性；耦合建模（§5.2），处理多子系统间的互动；在线参数辨识（§5.3），使模型能够自适应参数漂移；不确定性量化（§5.4），为鲁棒控制设计提供边界；以及面向不同水系统的建模策略对照表（§5.5），帮助工程师在实际工程中选择合适的建模方法。

这些技术是从第四章的"理想简化模型"升级到第七章"安全包络设计"和"全生命周期自主演进"的必要桥梁（参见第七章§7.4-7.9）。高级建模不是为了"追求精度"，而是为了确保在复杂和不确定的运行环境中，控制决策仍然是稳健和安全的。
...
> - 不确定性量化：建模误差的传播与边界
> - 鲁棒性余量设计：保守控制策略
> - 形式化检查清单确保第四章的形式化模型达到质量要求，为第七章的安全包络设计提供基础：质量保障工具
> 
> **可略读部分**（如已熟悉）：
...
ID/IDZ 模型建立在线性化假设之上，当水力工况偏离额定运行点时，非线性特性不可忽略。相关理论基础见第二章§2.4的可控可观性概念。本节介绍工程中处理非线性问题的四种常用策略，从简单到复杂依次递进。
...
### 5.1.3 扩展卡尔曼滤波处理状态估计非线性

水利系统的测量方程往往是非线性的（如基于水位-库容曲线推算出库流量），需要非线性状态估计工具。**扩展卡尔曼滤波（Extended Kalman Filter, EKF）** 通过对非线性方程在当前估计点进行 Jacobi 矩阵线性化，将标准卡尔曼滤波推广至非线性情形：
...
[5-1] Ljung L. System Identification: Theory for the User [M]. 2nd ed. Upper Saddle River, NJ: Prentice Hall, 1999.

[5-2] Julier S J, Uhlmann J K. Unscented filtering and nonlinear estimation [J]. Proceedings of the IEEE, 2004, 92(3): 401-422.

[5-3] Kalman R E. A new approach to linear filtering and prediction problems [J]. Journal of Basic Engineering, 1960, 82(1): 35-45.

[5-4] Söderström T, Stoica P. System Identification [M]. London: Prentice Hall, 1989.

[5-5] Garcia C E, Prett D M, Morari M. Model predictive control: Theory and practice—A survey [J]. Automatica, 1989, 25(3): 335-348.

[5-6] Bemporad A, Morari M. Robust model predictive control: A survey [M]//Robustness in Identification and Control. London: Springer, 1999: 207-226.

[5-7] 雷晓辉，王浩，许明，等。南水北调中线工程运行控制技术研究进展 [J]. 水利学报，2022, 53(3): 241-254.

[5-8] Litrico X, Fromion V. Modeling and Control of Hydrosystems [M]. London: Springer, 2009.

[5-9] Malaterre P O, Rogers D C, Schuurmans J. Classification of canal control algorithms [J]. Journal of Irrigation and Drainage Engineering, 1998, 124(1): 3-10.
