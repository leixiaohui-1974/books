...
1. 为什么水网调度中“约束处理能力”比“单点预测精度”更关键？
2. 解释预测时域 $T_p$ 与控制时域 $T_c$ 的功能差异。
3. 结合工程实践，说明滚动优化“只执行首个控制量”的意义。

### 应用题

4. 设计一个明渠场景的 MPC 目标函数，至少包含水位跟踪项、控制平滑项与安全惩罚项。  
5. 给定闸门动作上限与通信延时约束，构造一个可执行的上线门禁清单。

### 思考题

6. 当调度团队希望“快响应”，运维团队希望“少动作”时，你如何设计算法权重与治理流程？

### 基础题参考答案要点

1. 因上线系统首先要可安全运行，约束违背会直接造成风险；精度提升需在约束满足前提下讨论。  
2. $T_p$ 决定前瞻能力，$T_c$ 决定可调动作窗口；通常 $T_p\ge T_c$。  
3. 可提升鲁棒性并利用新测量持续纠偏，降低模型失配累积风险。
...
> **术语一致性声明**：本章统一采用“模型预测控制（Model Predictive Control, MPC）”“降阶模型（Reduced-Order Model, ROM）”“安全包络（Safety Envelope）”“运行设计域（Operational Design Domain, ODD）”“在环测试（In-the-Loop Testing）”等规范术语；数学符号采用 $Q, h, \tau, T_p, T_c, \mathbf{x}, \mathbf{u}, \mathbf{y}, J$ 统一记号。
