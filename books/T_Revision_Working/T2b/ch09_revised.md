<!-- 变更日志
v2 2026-03-03: 五书协同修订——强化§9.5.5自进化管线：明确影子模式（Shadow Mode）60天验证和OTA灰度发布为强制流程，对接T3第10章OTA升级审查规范和ch06 §6.9的详细规范；更新§9.7.2增量部署第五步的自进化约束说明
v1 2026-03-01: 基于 HydroClaw v0.2.2 代码全面重构，新增工作台层(§9.5)、认知API四维分类(§9.6)、增量部署(§9.7)；澄清L0 Core为T2a算法集成、策略表重构为OS管理视角
-->
...
> **交互日志** → *分析* → **改进建议** → *开发* → **新 Skill/知识** → *测试* → **门禁验证** → *影子模式（60天）* → *人类审计* → *OTA灰度发布* → **上线**

**关键约束一：xIL 测试门禁**。自进化生成的新 Skill 必须通过与手动开发的 Skill 相同的测试门禁（参见第十章 xIL 验证体系）——绝不能因为"自动生成"而降低质量标准。

**关键约束二：影子模式强制要求**。任何通过自进化产生的、涉及控制建议（决策维或控制维）的新策略，必须首先在**影子模式**下运行：

- 影子模式下，新策略与现有策略并行运行，但新策略不发出实际控制指令，只记录"如果执行，建议是什么"
- 影子建议与实际执行结果进行持续对比，评估新策略的质量和安全性
- 影子模式运行周期最少 **60 天**，满足五项量化门禁（详见第六章 §6.9.3 的完整规范）
- 60 天后经人类审计员审核，通过后才允许进入 OTA 灰度发布流程

**关键约束三：OTA 灰度发布**。通过审计的策略采用灰度发布方式上线：先部署到 5% 节点，7 天观测无异常后推进到 20%→50%→100%（详见第六章 §6.9.4）。全量部署后保留 30 天降级保护期。

**关键约束四：T3 对接要求**。对于影响物理控制网络（修改 MPC 参数、调整 ODD 边界）的自进化更新，必须额外满足 T3 第10章 OTA 升级审查流程的要求，包括安全评估报告和双层 ODD 一致性验证（参见第八章 §8.3.5）。
