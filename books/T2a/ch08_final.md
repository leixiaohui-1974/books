...
1. 硬约束不可违反（如安全水位上限），软约束可通过惩罚项折中（如经济性指标）。  
2. LP计算稳定且速度快，适合在线周期性求解与大规模调度。  
3. Pareto前沿表示在多目标下互不支配的最优折中解集合。

### 应用题

4. 为某泵站群写出一个“供水偏差+能耗+动作平滑”的多目标优化结构化表达。  
5. 某NLP求解耗时过长，请提出两种降复杂度策略并分析代价。

### 思考题

6. 在“全局最优性”与“实时可执行性”冲突时，你会如何制定工程上线优先级？
