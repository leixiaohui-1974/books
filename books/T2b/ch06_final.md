...
1. 因为调度建议必须可审计、可复核；无出处的回答无法作为生产决策依据。  
2. RAG侧重从文档中检索证据，知识图谱侧重实体关系与规则一致性，两者组合可同时提升可追溯性与推理一致性。  
3. 执行层联锁是安全底线；认知AI存在不确定性，必须经过规则校验与人工确认后才能影响控制动作。

> **术语一致性声明**：本章统一采用“运行设计域（Operational Design Domain, ODD）”“安全包络（Safety Envelope）”“认知AI引擎（Cognitive AI Engine）”“分层分布式控制（Hierarchical Distributed Control, HDC）”“多智能体系统（Multi-Agent System, MAS）”“SCADA+MAS Fusion Architecture”“水网操作系统（Water Network Operating System, HydroOS）”等规范术语，并与前序章节保持一致。
