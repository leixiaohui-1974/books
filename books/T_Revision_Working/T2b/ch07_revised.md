<!-- 变更日志
v3 2026-03-03: 五书协同修订——新增§7.4.5(断网自治的ODD收紧规则：降级时缩小可自主决策范围)、§7.4.6(MRC底层硬件接管与T3对接)；更新学习目标第6条；补充因果关系说明
...
6. 设计 SCADA+MAS 融合架构中的分层 Agent 体系，理解岛式自治（断网自治）中 ODD 收紧规则与 MRC 底层硬件接管的安全机制，以及与 T3 工程规范的对接。
...
第三阶段的 MAS 在 HDC 基础上增加了两个关键维度：**运行设计域（ODD）** 确保每个 Agent 在已验证的安全边界内运行（详见第八章），**认知智能** 赋予 Agent 语义理解和自主推理能力（详见第六章）。Lei 2025a 将这一关系总结为：

$$
\text{MAS} = \text{HDC} + \text{ODD} + \text{认知智能} \tag{7-1}
$$

这个公式的含义是：多智能体系统不是凭空构建的，而是在分层分布式控制这一成熟框架之上，叠加安全保障和认知能力。HDC 提供控制骨架，ODD 提供安全边界，认知智能提供灵活决策。三者缺一不可。
...
- **否决权**：当检测到安全边界被触碰时，L0 直接触发保护动作（紧急关闸、停泵），不需要等待上层 Agent 的确认；
...
### 7.4.5 断网自治中的 ODD 收紧规则

> **五书协同注**：本节对接 T3 第8章"硬件接管规范"中关于"自主范围随降级收缩"的要求。T3 规定：当系统从高等级降级时，自主决策范围必须相应缩小，不得在降级状态下维持高级别的自主权限。

**降级的物理直觉**：Agent 降级的根本原因是信息不完整。云 Agent 失联意味着全局态势感知缺失；域 Agent 失联意味着区域协调信息缺失；边缘 Agent 仅能依赖本地传感器。信息越残缺，可信决策范围越窄——这正是 ODD 收紧的逻辑依据。

**ODD 收紧的形式化定义**

设系统在正常运行状态（降级级别 0）的 ODD 为 $\mathcal{D}_0$，降级级别 $k$ 下的有效 ODD 为 $\mathcal{D}_k$。ODD 收紧原则要求：

$$
\mathcal{D}_3 \subsetneq \mathcal{D}_2 \subsetneq \mathcal{D}_1 \subsetneq \mathcal{D}_0 \tag{7-8}
$$

即降级级别越高，有效 ODD 越小（允许自主决策的范围越窄）。

具体而言，每次降级时，各维度的约束边界按以下规则收紧：

**表 7-5 降级级别与 ODD 收紧规则**

| 降级级别 | 失联范围 | 控制能力 | ODD 收紧内容 | 数学表达 |
|---------|---------|---------|------------|---------|
| 0（正常） | 无 | 全局 MAS 优化 | 完整 ODD | $\mathcal{D}_0 = \mathcal{D}^{norm}$ |
| 1（云断） | 云↔域 | 域内 DMPC | 禁止全网跨域操作；本域内 ODD 不变 | $\mathcal{D}_1^{domain} = \mathcal{D}_0^{domain}$，全局协调暂停 |
| 2（域断） | 域↔边缘 | 本地 PID | 约束收紧 10%；禁止主动调整执行器超出当前值 ±5% | $[l_i^2, u_i^2] = [l_i^0 + 0.1(u_i^0 - l_i^0)/2, \; u_i^0 - 0.1(u_i^0-l_i^0)/2]$ |
| 3（全断） | 全 | PLC 联锁 | 仅允许硬件联锁动作；冻结最后安全设定值 | $\mathcal{D}_3 = \{x : x = x^{last\_safe}\}$（理论上的单点集）|

> **工程注记**：$\mathcal{D}_3$ 在理论上定义为单点集（仅允许在最后安全设定值处），但在工程实践中，PLC 联锁逻辑通常保留一个小的死区（dead-band），例如 $\pm 0.02$ m 的水位波动容忍带，以避免联锁控制器因微小的传感器噪声而持续震荡触发。此死区宽度应在系统设计时根据传感器精度和执行器分辨率确定，并在 T3 工程规范中明确记录。

**示例**：某渠池水位的 ODD 正常范围为 $h \in [1.0, 3.5]$ m，降级至级别 2（域-边缘断连）后：

$$
l_2 = 1.0 + 0.1 \times \frac{3.5-1.0}{2} = 1.125 \text{ m}, \quad u_2 = 3.5 - 0.1 \times \frac{3.5-1.0}{2} = 3.375 \text{ m}
$$

即降级后的有效 ODD 为 $h \in [1.125, 3.375]$ m，比正常时收紧了约 5%。

**ODD 收紧的工程含义**：收紧并非意味着"边缘 Agent 无所作为"，而是意味着边缘 Agent 的**自主决策空间受限**。在收紧后的 ODD 内，边缘 Agent 仍然可以自主调节（通过 PID 维持水位）；但若系统状态接近收紧后的边界，边缘 Agent 必须立即触发 MRC（最小风险状态），而不是像正常模式下那样先进入黄区等待人工确认。这保证了降级状态下的更保守行为。

**恢复时的 ODD 扩展**：通信恢复后，ODD 按照降级逆过程逐步扩展，而非立即跳回完整 ODD：

```
通信恢复 → 状态一致性检查 → 确认无积累偏差
    → ODD 逐步扩展（每 5 分钟扩展 20%）
    → 5 步后完全恢复 $\mathcal{D}_0$
```

> **工程注记**：每次 ODD 扩展步骤（每步 +20%）前，必须完成快速回归验证（运行 MIL 级别的功能性检查），确认扩展后的新边界内系统行为正常，而非仅依赖计时器自动扩展。若任一步验证失败，扩展暂停并保持当前收紧的 ODD，等待人工审查。此要求防止通信恢复后在"正常超时→自动扩展"机制下跳过安全检查。

此设计防止"通信恢复瞬间，系统从极保守的 $\mathcal{D}_3$ 突然跳到完整 $\mathcal{D}_0$"导致控制器行为突变。

[图7-2: 断网自治 ODD 收紧规则示意图]
{描述: 同心椭圆形嵌套图。最外层（灰色虚线）：正常 ODD $\mathcal{D}_0$，标注"L0正常-全局MAS优化"。第二层（蓝色虚线）：降级1 $\mathcal{D}_1$，标注"L1云断-域内DMPC"。第三层（橙色实线）：降级2 $\mathcal{D}_2$，标注"L2域断-本地PID，边界收紧10%"。最内层（红色实心）：降级3 $\mathcal{D}_3$，标注"L3全断-PLC联锁，冻结安全值"。右侧箭头标注"通信恢复→逐步扩展（5步×5min）"。}
{尺寸: 半页}
{颜色方案: 灰/蓝/橙/红四层嵌套，由外到内}

### 7.4.6 MRC 底层硬件接管：与 T3 工程规范的对接

> **五书协同注**：本节对接 T3 第8章"MRC底层硬件接管规范"。T3 规定：当软件层（包括 MAS 的所有 Agent）无法保证安全时，必须触发底层硬件接管，由 PLC 直接执行预定义的安全动作。本节从认知AI角度解释何时、如何移交控制权给底层硬件。

#### 7.4.6.1 三层安全底板的接管逻辑

MAS 架构中存在三层安全底板，构成纵深防御（Defense in Depth）体系：

**第一层：SafetyAgent（软件层）**
- 主体：T2b SafetyAgent（第六章 §6.5.4）
- 响应速度：秒级
- 职责：ODD 三区间监测、否决不安全操作建议、生成结构化因果树
- 接管权：否决上层 Agent 决策，但不直接操控执行器

**第二层：边缘 Agent 本地 PID（嵌入式软件层）**
- 主体：部署在边缘设备的本地控制器
- 响应速度：秒级
- 职责：在域-边缘断连时维持本地水位稳定
- 接管权：域 Agent 的控制权下放，但受 SafetyAgent 约束

**第三层：PLC 硬件联锁（底层硬件层）**
- 主体：可编程逻辑控制器（PLC）的硬连线联锁逻辑
- 响应速度：毫秒级
- 职责：T3 规定的底层安全动作
- 接管权：**绝对权威**，无法被任何软件层覆盖
...
```
条件 1: 状态进入 ODD MRC 区
    → SafetyAgent 否决所有建议，生成软件层 MRC 方案
    → 若软件 MRC 方案被执行后 5 分钟内状态未改善
        → 触发条件 2

条件 2: 软件 MRC 失效 OR 所有 Agent 离线
    → 边缘 Agent 切换到保守 PID 模式（ODD 收紧）
    → 若 PID 模式下状态仍越界（跌出 D_2）
        → 触发条件 3

条件 3: 状态超出边缘 Agent 安全处置能力
    → PLC 硬件联锁接管
    → 执行 T3 §8.3 规定的预定义安全动作序列
    → 发送 HANDOVER_REQUEST 给人类操作员（含结构化因果树）
```

#### 7.4.6.2 T3 规定的硬件接管动作序列

T3 第8章规定，水网 PLC 硬件接管后的默认动作序列分三档（按危险程度升级）：

| 危险档位 | 触发条件 | PLC 执行的动作 | 自主恢复条件 |
|---------|---------|-------------|------------|
| 档位 A（告警级） | 1个维度进入 MRC 区 | 降低入流 30%；开启溢洪通道 | 状态回到扩展域后 5 分钟 |
| 档位 B（紧急级） | 2+维度进入 MRC 区或水位>设计上限 | 关闭上游闸门；停止泵站；开启所有溢洪通道 | 需人工复位 |
| 档位 C（灾难级） | 检测到设备失控信号或传感器全断 | 关闭全部可控执行器至预定安全位；启动应急广播 | 需人工现场复位 |

**接管移交协议**：硬件接管时，PLC 同步向人机界面推送接管通知，内容必须包含：
1. 触发档位（A/B/C）
2. 触发时刻的传感器快照
3. PLC 已执行的动作列表
4. 请求人类操作员到场的位置和操作码

这与第六章 §6.5.7 的结构化因果树形成**上下游对接**：SafetyAgent 的因果树在软件层触发时推送，PLC 的接管通知在硬件层触发时推送，两者共同构成完整的事故追溯链。

#### 7.4.6.3 从 Agent 视角看待硬件接管

从认知AI的角度，底层硬件接管是一个**不可预测的边界条件**——MAS 的所有 Agent 都需要知道"当 PLC 接管后，我应该怎么办"：

- **SafetyAgent**：停止发出否决决策（PLC 已接管，软件否决无意义）；进入"观察记录"模式，持续记录 PLC 的执行过程以备审计
- **其余 Agent**：暂停所有操作建议；等待硬件接管结束信号；接管结束后进行状态一致性检查，确认 MAS 的状态估计与实际状态重新对齐后才恢复操作
- **Orchestrator**：广播 `HARDWARE_TAKEOVER` 事件到所有 Agent；记录接管开始时间戳；接管结束后执行"重启序列"（按降级逆过程重新上线各 Agent）

```python
class OrchestratorHardwareTakeoverHandler:
    """Orchestrator 对 PLC 接管事件的响应"""

    def on_hardware_takeover_started(self, event: HardwareTakeoverEvent):
        # 广播接管通知
        self.message_bus.broadcast("HARDWARE_TAKEOVER", {
            "level": event.level,           # A/B/C
            "trigger_time": event.timestamp,
            "paging_all_agents": True
        })
        # 冻结所有 Agent 的操作输出
        for agent in self.registry.all_agents():
            agent.set_output_frozen(True)
        # 激活全程记录模式
        self.audit_logger.enter_takeover_recording_mode()

    def on_hardware_takeover_ended(self, restored_state: SystemState):
        # 执行重启序列（按降级逆过程）
        self._restart_sequence(restored_state)

    def _restart_sequence(self, state: SystemState):
        """从降级级别 3 逐步恢复到级别 0"""
        # 1. 先恢复边缘 Agent（级别 2→3 反向）
        for edge_agent in self.registry.edge_agents():
            edge_agent.validate_state_consistency(state)
            edge_agent.set_output_frozen(False)
        # 2. 域 Agent 在收到所有边缘 Agent 就绪确认后恢复
        # 3. 云 Agent 最后恢复，ODD 逐步扩展
        # (完整逻辑见 §7.4.5 恢复机制)
```
...
**表 7-4 MAS 理论概念与 HydroClaw 实现对照**
...
在**架构维度**，我们介绍了 SCADA+MAS 三层融合架构（现场/协调/认知）、云-域-边 Agent 层级、L0 安全底板的绝对否决权、岛式自治的逐级降级策略，以及断网自治中的 **ODD 收紧规则**（降级时自主决策范围随信息残缺程度收缩，$\mathcal{D}_3 \subsetneq \mathcal{D}_2 \subsetneq \mathcal{D}_1 \subsetneq \mathcal{D}_0$）和 **MRC 底层硬件接管协议**（三层纵深防御：SafetyAgent → 边缘 PID → PLC 硬连线，与 T3 第8章工程规范对接）。
...
7. 岛式自治中的 **ODD 收紧规则**（式7-8）：信息越残缺，自主决策范围越窄，降级级别越高，有效 ODD 越小；
8. **三层纵深防御**（SafetyAgent→边缘PID→PLC硬连线）确保任何单点失效不会导致全系统失控，PLC 硬件接管对应 T3 第8章 MRC 规范的工程实现。
