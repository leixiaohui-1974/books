...
**2-1.** 为什么时序任务不宜随机切分训练集与测试集？  
**2-2.** SVM、RF、GBDT 的典型优势分别是什么？  
**2-3.** 为什么模型上线必须同时满足精度门禁与治理门禁？


### 基础题参考答案要点

**2-4.** 随机切分会导致未来信息泄漏到训练集，造成评估结果虚高并误导上线决策。  
**2-5.** SVM适合小样本高维任务，RF鲁棒抗噪，GBDT在结构化特征上通常表现更优。  
**2-6.** 仅有精度不代表可安全运行，治理门禁（版本、审计、回退）决定工程可控性。

### 应用题

**2-7.** 设计一个用于水位预测的特征工程方案（至少包含5类特征）。  
**2-8.** 设计“模型误差升高时的降级与回退”运行流程。

### 思考题

**2-9.** 在模型精度略优但解释性明显更差时，工程上线应如何决策？
