...
> 
> **与后章的关系**：本章的可观性判据是第七章八原理中"原理二：可控可观性"的工程实施指南，也是第九章在环验证的前提条件。
> 
> **核心概念**（5 个）：
...
可验证性判据直接对接第七章原理五"在环验证"（工程实现详见第九章）。任何控制策略上线运行前，至少完成三级验证：
...
$$
\dot{\hat{x}} = A\hat{x} + Bu + L(y - C\hat{x}) \tag{6-12}
$$
...
> **实例性质声明**：§7.6和§7.7的工程参数（管网规模、传感器数量、优化效果等）为**说明性设定**，旨在演示可观性分析和传感器布局优化的完整方法流程，不对应特定工程的实测数据。参数量级参照多个实际工程设定，具有工程合理性。
...
基于 IDZ 传递函数模型 [6-5]，建立线性状态空间模型：

$$
x_{k+1} = A_d x_k + B_d u_k, \quad y_k = C_s x_k + v_k \tag{6-13}
$$
...
$$
W_o = \sum_{k=0}^{N_T} (A_d^k)^\top C_s^\top C_s A_d^k \tag{6-14}
$$
...
**工程后果**：MPC 控制器基于错误模型参数运行，预测精度下降，控制输出出现振荡或过调。长距离输水工程冬季运行时此问题尤为突出（第十五章详述冰期参数在线辨识方案）。
...
跨流域输水工程涉及数百公里管渠、数十座泵闸、数百个测量点，直接对完整系统进行可控可观性矩阵分析在计算上不可行（状态维度 $n  \gt  500$ 时矩阵操作耗时指数增长）。本节介绍三种工程可行的分解方法。
...
$$
W_o = \sum_{k=0}^{N} (A^k)^\top C^\top C A^k \tag{6-15}
$$

可通过 Lyapunov 方程求解：

$$
A^\top W_o A - W_o + C^\top C = 0 \tag{6-16}
$$
...
$$
\Sigma = \frac{1}{T}\sum_{k} x(k)x(k)^\top \tag{6-17}
$$
...
> **与 CHS 体系的联系**：大规模系统的可控可观性分解分析是 HydroOS 调度与智能层（SIL，第十一章）中"状态估计服务"设计的理论依据，决定了哪些状态需要全系统协调估计、哪些状态可在子区域内独立估计。
...
雷晓辉（Lei 2025d）在本书配套论文《基于无人驾驶理念的下一代自主运行智慧水网架构与关键技术》（南水北调与水利科技，2025 年第 4 期）中，从 CHS 八原理视角阐释了水系统可控可观性的工程意义，是本章内容的系统性学术论证，建议对照阅读 [6-9]。

**学科展望**：

雷晓辉（Lei 2025c）在《水资源系统分析学科展望：从静态平衡到动态控制》中，从学科演进视角分析了可控可观性理论在水资源系统分析中的核心地位，揭示了从静态配置到动态控制范式转变的学科驱动力 [6-10]。
...
> **术语对照**：T2b第九章从软件工程视角将HydroOS三层命名为DAL（设备抽象层）、SIL（调度与智能层）、SAL（服务与应用层）。本节沿用T1-CN第十一章的能力视角术语——PAI（物理AI引擎，对应SIL）和CAI（认知AI引擎，对应SAL），以突出可控可观性理论与各层能力的映射关系。
...
**与后续章节的衔接**：本章建立的可控可观性工程框架是后续章节的重要铺垫。第七章八原理中的"原理二（可控可观性）"将从学科理论层面阐释本章工程判据的思想来源；第九章安全包络与在环验证将以本章的三级验证框架（MIL/SIL/HIL）为基础，详细展开验证场景设计和评判准则；第十二至十四章的工程案例将展示本章理论与方法的完整应用过程。
...
[6-10] 雷晓辉,许慧敏,何中政,等.水资源系统分析学科展望：从静态平衡到动态控制[J].南水北调与水利科技(中英文),2025,23(04):770-777.DOI:10.13476/j.cnki.nsbdqk.2025.0078.
