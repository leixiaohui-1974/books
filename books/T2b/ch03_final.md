...
## 例题：LSTM与Transformer在入库流量预测中的比较
...
**3-1.** CNN、LSTM、Transformer 在水系统任务中的典型适用场景分别是什么？  
**3-2.** 为什么深度模型需要报告高分位误差而不仅是平均误差？  
**3-3.** 迁移学习在跨流域应用时最先应检查什么？


### 基础题参考答案要点

**3-4.** CNN偏空间特征提取，LSTM偏时序记忆建模，Transformer适合长依赖与多变量耦合。  
**3-5.** 平均误差会掩盖极端风险，高分位误差更能反映安全关键时段性能。  
**3-6.** 首先检查输入分布与工况边界是否一致，再决定迁移/微调策略。

### 应用题

**3-7.** 设计一个用于多站点水位预测的深度学习数据管线。  
**3-8.** 给出“深度模型上线失败后的回退与复盘”流程。

### 思考题

**3-9.** 当 Transformer 精度更高但算力成本显著增加时，工程上线应如何权衡？
