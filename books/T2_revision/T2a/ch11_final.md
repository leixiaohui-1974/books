...
> **章首衔接（承接ch10）**  
> 上一章定义了安全包络与联锁规则。本章回答“边界判定依据从何而来”：通过状态估计与数据融合，为控制与安全监测提供可信状态。
...
**11-1.** 为什么水系统控制不能只依赖原始测点值？  
**11-2.** EKF 与 UKF 的核心差异是什么？  
**11-3.** 解释残差 \(r_k\) 在异常检测中的作用。

### 基础题参考答案要点

**11-4.** 原始测点不覆盖完整系统状态，且含噪声与缺测，需估计器融合模型与观测获得可控可观的状态输入。  
**11-5.** EKF依赖局部线性化与雅可比矩阵；UKF通过sigma点传播统计量，通常在非线性较强时更稳健。  
**11-6.** 残差反映观测与预测偏差，是故障检测、阈值触发与降级切换的重要依据。

### 应用题

**11-7.** 为一个“3测点+1缺测冗余点”泵站设计简化融合流程。  
**11-8.** 设计一次“传感器漂移”故障注入实验，并给出处置触发条件。

### 思考题

**11-9.** 当估计精度与实时性冲突时，如何在工程上做取舍？
