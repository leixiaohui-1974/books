...
> 
> **与后章的关系**：本章的可观性判据是第三章八原理中"原理二：可控可观性"的工程实施指南，也是第三章在环验证的前提条件。
> 
> **核心概念**（5 个）：
...
可验证性判据直接对接第三章原理五"在环验证"（工程实现详见第十一章）。任何控制策略上线运行前，至少完成三级验证：
...
> **实例性质声明**：§6.6和§6.7的工程参数（管网规模、传感器数量、优化效果等）为**说明性设定**，旨在演示可观性分析和传感器布局优化的完整方法流程，不对应特定工程的实测数据。参数量级参照多个实际工程设定，具有工程合理性。§6.13的胶东调水案例基于公开文献和实际工程原型。
...
针对 571km 超长线路，采用§6.10.1的子系统分解方法，按泵站加压区间划分为 7 个分析子区：
...
**从胶东调水案例到通用设计原则**：胶东调水工程的可观性优化实践，可以提炼为长距离调水工程传感器布局的三条通用原则：① 泵站边界原则——泵站进出口两侧是水力不连续点，是可观性"节点"，必须直接测量而非估计；② 分水口零遗漏原则——所有分水口必须配置流量计，否则水量平衡方程中存在不可消除的误差项，所有下游估计精度均受其制约；③ 渠段特征点原则——每个独立控制渠池至少保留目标断面的直接测量，其余可依赖估计，但"至少一个"是保证闭环稳定的基本要求。这三条原则与 §6.2.2 双通道观测判据的精神完全一致，是后者在长距离调水场景下的工程化表达。
...
**第二条主线：传感器布局优化**。通过可观性格拉姆矩阵最大化框架，可以定量回答"哪里需要传感器、需要多少个"。§6.3.2 介绍了贪婪算法、遗传算法、混合整数规划三种求解方法，适用于不同规模的工程问题。§6.6–5.7 的两个工程实例表明：与均匀布设相比，优化布局在削减 40%–65% 传感器数量的同时，可将状态估计 RMSE 提升 40%–70%，年运维费用节省 20%–50%。优化的核心逻辑是"以信息价值密度而非空间密度为导向"。

**第三条主线：数学工具的工程化**。§6.5 的数学判据是工程判据的理论支撑：可控性矩阵满秩保证 MPC 中极点配置的可行性，可观性矩阵满秩保证 EKF 的渐近收敛，格拉姆矩阵的最小特征值量化"最难控制/最难观测"的方向，Luenberger 观测器为无传感器断面提供状态重构方法。§6.10 的分解分析方法（子系统分解、稀疏矩阵、PCA 降维）解决了大规模水网中矩阵计算不可行的实际困难。

**第四条主线：运行期的动态维护**。§6.9 的四类缺陷模式（执行器布局不当、传感器缺失、通信故障、季节性退化）揭示了可控可观性不是一次性设计结果，而是随工程状态动态变化的系统属性。§6.15 的六类典型误区进一步强调，可控可观性分析需贯穿系统全生命周期：设计期建立基准指标，运行期通过在线监控台账追踪退化，维护期基于量化评估决策传感器增减和执行器更新。

**与后续章节的衔接**：本章建立的可控可观性工程框架是后续章节的重要铺垫。第三章八原理中的"原理二（可控可观性）"将从学科理论层面阐释本章工程判据的思想来源；第十一章安全包络与在环验证将以本章的三级验证框架（MIL/SIL/HIL）为基础，详细展开验证场景设计和评判准则；第十二至十四章的三个工程案例将在南水北调、大渡河梯级、胶东调水等具体工程背景下，展示本章理论与方法的完整应用过程。
...
8. **可控可观性分析实践**：选择你熟悉的水利工程（水库/渠道/管网均可），完成以下任务：
 ...
 - (d) 基于 §6.15 的六类典型误区，识别该工程中存在哪些潜在误区
