...
[物理直觉] 水系统不是“越快越好”，而是“在安全包络内稳定运行”。安全包络用于定义允许运行状态集合，超出边界即触发联锁或降级。
...
[工程解释] \(\sigma(\mathbf{x})\) 表示到最近安全包络边界的距离，适合用于在线监视与预警分级。
...
**10-1.** 解释 ODD 与 Safety Envelope 的关系。  
**10-2.** 红/黄/绿三区间分别对应何种控制策略？  
**10-3.** 为什么最小风险状态必须预先定义？

### 基础题参考答案要点

**10-4.** ODD 是已验证运行域，Safety Envelope 是其中更严格的安全子域。  
**10-5.** 绿色执行常规优化，黄色收缩约束并保守控制，红色触发联锁与接管。  
**10-6.** 以确保越界时可快速转入可维持、可审计的低风险运行形态。

### 应用题

**10-7.** 为某泵站系统设计一套三色阈值与联锁触发规则。  
**10-8.** 给出一个在环测试方案，用于验证安全联锁时效性与可复位性。

### 思考题

**10-9.** 当“供水连续性”与“安全包络严格性”冲突时，应如何设定优先级？
