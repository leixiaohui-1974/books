...
1. 为什么线性化模型仍可用于明渠MPC的工程部署？
2. 解释 $T_p$、$T_c$、采样周期三者的协同关系。
3. $Q_y$ 与 $R_u$ 的调节分别会带来什么控制表现变化？

### 应用题

4. 给定一组明渠状态空间模型与约束，写出QP标准形式中的 $\mathbf{H},\mathbf{f},\mathbf{G},\mathbf{b}$ 组成思路。  
5. 设计“性能参数组/保守参数组”的切换规则，并给出触发阈值建议。

### 思考题

6. 在极端工况下，若线性MPC频繁触发回退，你会优先改进模型、约束、还是调度目标？请给出决策依据。

### 基础题参考答案要点

1. 因为在局部运行区间内线性近似可捕捉主导动态，且在线求解效率高。  
2. 采样周期决定控制刷新频率，$T_p$ 决定前瞻深度，$T_c$ 决定可调动作窗口。  
3. 增大 $Q_y$ 强化跟踪，增大 $R_u$ 强化平滑；两者需平衡。
...
> **术语一致性声明**：本章统一采用“模型预测控制（Model Predictive Control, MPC）”“安全包络（Safety Envelope）”“运行设计域（Operational Design Domain, ODD）”“分层分布式控制（Hierarchical Distributed Control, HDC）”等规范术语；数学符号采用 $Q,h,\tau,T_p,T_c,\mathbf{x},\mathbf{u},\mathbf{y},\mathbf{A,B,C,D},J$。

### 质量检查清单（执行结果）

- [x] 章首学习目标、章末小结/习题/拓展阅读完整；
- [x] 基础题3 + 应用题2 + 思考题1，含基础题答案要点；
- [x] 每个公式前有物理直觉或上下文解释，后有工程解释；
- [x] 含可复现实例（例3-1）；
- [x] 图表不少于2项（图3-1、表3-1）；
- [x] 章首回顾ch02，章末预告ch04。
