...
1. 频域辨识擅长频段特性提取，但闭环性能仍需时域验证；两者关注点不同且互补。  
2. 相干函数用于衡量输入输出在各频段的线性相关与数据可信度。  
3. 常对应纯滞后（传输延时）主导的相位贡献。

> **术语一致性声明**：本章统一采用“降阶模型（Reduced-Order Model, ROM）”“传递函数（Transfer Function）”“运行设计域（Operational Design Domain, ODD）”“安全包络（Safety Envelope）”“模型预测控制（Model Predictive Control, MPC）”“在环测试（In-the-Loop Testing）”等规范术语；数学符号使用 $Q, h, A, S_0, S_f, n, c, \tau, G(s)$ 等统一记号。

### 质量检查清单（执行结果）

- [x] 章首学习目标与章末小结/习题/拓展阅读完整；  
- [x] 基础题3 + 应用题2 + 思考题1，且补基础题答案要点；  
- [x] 章首回顾上一章结论，章末预告下一章内容；  
- [x] 关键公式均配“物理直觉”或“工程解释”；  
- [x] 图表占位不少于2项（图6-1、图6-2、表6-1）；  
- [x] 包含可复现实例（例6-1）与上线门禁说明。
