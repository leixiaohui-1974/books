<!-- 变更日志
v4 2026-03-03: 三角色审改(A:9.0/B:9.0/C:8.5)——§5.5.3增加ch07 MAS通信协议交叉引用；§5.6.3末尾增加ch06 SafetyAgent交叉引用；策略蒸馏补充参考文献Hinton et al.(2015)；SAC自动调节温度机制简述
...
| 奖励项 $J_k$ | 工程含义 | 典型形式 |
|:---|:---|:---|
| $J_{\text{tracking}}$ | 目标跟踪偏差 | $\|h - h_{\text{ref}}\|^2$ 或 $\|Q - Q_{\text{target}}\|^2$ |
...
| $J_{\text{switch}}$ | 动作平滑 | $\|\Delta u_t\|^2$（抑制频繁切换） |
...
**多智能体 RL 方案**：每个电站部署一个 RL 智能体，通过通信共享局部状态（多智能体系统的通信协议和协同机制详见 ch07 多智能体系统理论）。与集中式 RL 相比，多智能体方案更贴合水系统的分布式运行架构，且更容易扩展。
...
**最小风险状态（Minimum Risk State, MRS）**是 Lei 2025d 提出的水网自主运行架构中的核心安全概念——当系统检测到超出 ODD 的工况或策略不确定性过高时，自动切换到预定义的最保守运行模式。
...
**一键回退链路**：从 RL 策略到 MPC 基线的切换必须在单个控制周期内完成（通常 < 1 秒），确保切换过程不产生控制真空。在 HydroClaw 认知AI引擎的实现中，MRS 的触发和执行由 SafetyAgent 负责——SafetyAgent 持续监测 ODD 状态，一旦判定红区即刻执行否决和 MRC 方案（详见 ch06 §6.5.4 SafetyAgent 的 ODD 三区间否决权机制）。
...
离线训练通过后，按 Lei 2025b 提出的在环测试体系进行验证：
...
**结果讨论**：对于样本成本高（如仿真慢）的水系统，优先推荐 SAC。对于可大规模并行仿真的场景（如简化模型），PPO 也是良好选择。
...
步骤 4：在 SIL 中测试——用 1998 年特大洪水过程重现，验证三层约束在极端条件下的协同表现。
...
[5-3] 雷晓辉,许慧敏,何中政,等.基于无人驾驶理念的下一代自主运行智慧水网架构与关键技术[J].南水北调与水利科技(中英文),2025,23(04):778-786+905.DOI:10.13476/j.cnki.nsbdqk.2025.0079.
