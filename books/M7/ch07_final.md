...
**7-1.** 为什么门禁规则必须分层而不是单一阈值判断？
**7-2.** major/minor/patch 三类门禁规则变更在审批上有何差异？
**7-3.** 解释门禁时序预算公式中四项时延含义。

### 应用题

**7-4.** 设计一份门禁规则变更审批矩阵，给出不同变更级别的签批角色与时限。  
**7-5.** 设计审计事件最小字段清单，并说明哪些字段用于责任追溯。

### 思考题

**7-6.** 在极端应急场景下，是否允许“临时越权放行”？若允许，应设置哪些补偿性审计机制？

### 基础题参考答案要点

**7-7.** 因权限、运行约束和联锁风险属于不同风险层次，需分层校验。  
**7-8.** major需最高级多方审批，minor中等级双签，patch可简化备案。  
**7-9.** 分别对应认证、规则计算、风险评估、审计确认，总和受门禁预算约束。
