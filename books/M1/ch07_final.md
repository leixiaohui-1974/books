...
1. 因为数据模型存在外推风险，需通过 ODD 与 Safety Envelope 约束其适用边界与失效回退。  
2. 子空间法侧重状态降维与主模态保持；残差学习法侧重补偿机理模型误差。  
3. 会高估模型性能，导致上线后误差显著反弹。

> **术语一致性声明**：本章统一采用“降阶模型（Reduced-Order Model, ROM）”“运行设计域（Operational Design Domain, ODD）”“安全包络（Safety Envelope）”“模型预测控制（Model Predictive Control, MPC）”“在环测试（In-the-Loop Testing）”“最小风险状态（MRS）”等规范术语；数学符号使用 $Q, h, A, S_0, S_f, n, c, \tau, G(s)$ 等统一记号。

### 质量检查清单（执行结果）

- [x] 学习目标、正文、例题、小结、习题、拓展阅读完整；  
- [x] 基础题3 + 应用题2 + 思考题1，并含基础题答案要点；  
- [x] 章首衔接上一章，章末预告下一章；  
- [x] 图表占位不少于2项（图7-1、图7-2、表7-1）；  
- [x] 可复现实例（例7-1）含计算与工程讨论；  
- [x] 术语与符号符合 CHS 规范。
