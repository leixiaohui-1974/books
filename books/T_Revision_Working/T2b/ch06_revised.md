<!-- 变更日志
v3 2026-03-03: 五书协同修订——新增§6.5.6(SafetyAgent↔ODD边界监测器：MPC快思考执行与LLM慢思考规划的接口)、§6.5.7(结构化因果树：接管请求规范化输出，对接T3第13章)、§6.8(自进化管线：影子模式与OTA升级，对接T3第10章)；更新学习目标第7条；补充相关例题
...
7. 论述 SafetyAgent（ODD边界监测器）作为MPC快思考与LLM慢思考之间安全过滤接口的架构角色，以及结构化因果树在接管请求规范化中的作用。
...
两个引擎不是替代关系，而是互补关系。物理AI引擎提供可信的数值计算，认知AI引擎提供可追溯的语义推理。只有两者协同，才能构建真正意义上的自主水网（Lei 2025d）。
...
这种分层架构的设计哲学是**"不同层次的确定性对应不同层次的自主权"**：SCADA层（最确定）有最高的自主权（自动执行联锁）；规则层有条件的自主权（在规则覆盖范围内自动执行）；认知AI层有最低的自主权（需要人工确认后才能执行）。确定性越低，人工监督越严格——这与 CHS 理论中 WNAL 自治等级的核心思想一脉相承（Lei 2025d）。
...
这个隐喻完美映射到水网认知AI的架构设计中。水利工程运行中，大部分日常任务是标准化的：每天做一次水位预报、每小时检查一次安全边界、每班做一次交接摘要。这些任务的步骤固定、工具确定、流程不变——正如人类的快思考。但偶尔会出现异常：复合告警、设备联锁故障、超出规程范围的极端水情。这些场景需要灵活推理、跨域组合、创造性应对——这正是慢思考的领域。
...
{描述: 纵向五层金字塔，从底层L0(蓝灰)到顶层L4(深蓝)渐变。每层标注层名、功能、代表组件。右侧标注"Tool→Skill→Agent"三级抽象的对应关系。左侧标注数据流方向（自下而上）和控制流方向（自上而下）。底部L0最宽表示基础最稳固，顶部L4最窄表示抽象最高。}
{尺寸: 全页}
{颜色方案: 蓝色系渐变}
...
为了帮助读者把 HydroClaw 的实现与两册教材的知识结构对齐，同时明确每一层对 WNAL（Water Network Autonomy Level）自治等级的支撑作用，可参照表 6-3 所示的映射（Lei 2025d）。

[表6-3: HydroClaw 层级与教材/自治等级映射]
...
[图6-2: MCP 工具协议三要素]
{描述: 三列布局。左列"工具描述"（JSON Schema卡片），中列"工具发现"（注册中心与服务器连线），右列"工具调用"（请求→处理→响应→日志的流水线）。底部标注三者的关系：描述使发现成为可能，发现使调用有据可依。}
{尺寸: 全页}
{颜色方案: 蓝色系}
...
**预警（Warning）**：将预测值与 ODD 安全边界对比，输出预警等级：
- `none`（绿色）：所有指标在正常范围内
...
[图6-3: 四预闭环条件链路图]
{描述: 从左到右的流程图，四个步骤用蓝色圆角矩形表示。预报→预警之间用实线箭头连接。预警之后有一个菱形判断节点"预警等级≥黄色?"，"否"分支指向绿色"正常"终止符，"是"分支指向预演→预案。每个步骤下方标注调用的MCP工具名。顶部标注"条件链接：逐级触发，按需执行"。}
{尺寸: 全页}
{颜色方案: 蓝色为主，判断节点用橙色，正常终止用绿色}
...
{描述: 漏斗形流程图，从上到下四层。用户请求从顶部进入。第一层（浅蓝）"Skill短语匹配"——最宽，覆盖大部分日常请求。第二层（中蓝）"工具关键词"。第三层（深蓝）"能力路由"。第四层（最深蓝）"Planning分解"——最窄，仅用于复杂请求。右侧标注每层的响应速度：毫秒→秒→秒→分钟。左侧标注：越往上=越快越确定，越往下=越慢越灵活。}
{尺寸: 半页}
{颜色方案: 蓝色系渐变}
...
{描述: 水平流程图。左侧：任意Agent提出操作建议。中间：SafetyAgent接收建议，调用check_odd检查当前状态。右侧三条分支：上方绿色箭头"normal→放行，执行操作"；中间黄色箭头"extended→人工确认对话框"；下方红色箭头"mrc→否决❌ + MRC方案"。红色分支用粗线条强调。底部标注："SafetyAgent拥有绝对否决权，高于所有Agent"。}
{尺寸: 全页}
{颜色方案: 绿/黄/红三色信号灯}
...
### 6.5.6 SafetyAgent 作为 MPC 快思考与 LLM 慢思考的安全接口

> **五书协同注**：本节阐述认知AI（T2b）与安全工程（T3）的深度对接。T3 第8章在硬件层面定义了 ODD 边界监测器的实时接管规范；本节在认知AI层面给出其软件对应——SafetyAgent 如何过滤 LLM（慢思考）发出的调度建议，确保最终执行的控制指令始终在 MPC（快思考）的物理可行域内。

#### 6.5.6.1 快思考与慢思考的分工

水系统认知AI中存在**两类决策时钟**，对应 Kahneman（2011）的"快慢思考"框架：

| 决策层 | 实现 | 典型周期 | 决策依据 | 用途 |
|--------|------|---------|---------|------|
| **快思考执行层** | MPC 控制器（T2a 第七章） | 秒—分钟 | 物理模型 + 当前传感器量测 | 闸门/泵站实时调节，保证水力稳定 |
| **慢思考规划层** | LLM / Agent（本章 §6.5） | 分钟—小时 | 知识库 + 运行规程 + 历史案例 | 生成调度建议方案、应急预案 |

**核心问题**：慢思考（LLM）的调度建议如何安全地交给快思考（MPC）执行？直接传递存在两类风险：

- **物理越界风险**：LLM 可能建议"提高节点 A 流量至 Q = 2.5 m³/s"，但 A 的下游渠池当前水位接近溢出阈值——MPC 若照单全收，可能导致溢坝。
- **时序失配风险**：LLM 的建议是基于分钟前的状态快照生成的；若中间状态已突变（如突发支流入流），建议已经过时。

**SafetyAgent（ODD 边界监测器）** 正是解决这两类风险的"安全过滤器"。它充当慢思考与快思考之间的**守门人**，实现以下三步过滤：

```
LLM/Agent（慢思考）
    │
    ▼  调度建议（方案级）
SafetyAgent → ODD 边界监测器
    │
    ├── 绿区：建议可直接作为 MPC 的参考轨迹/约束
    │         → MPC（快思考）执行
    │
    ├── 黄区：建议部分越出正常域
    │         → 修剪约束（Constraint Trimming）后传递给 MPC
    │         → 同时触发人工确认弹窗
    │
    └── 红区：建议可能导致 MRC 触发
              → 否决建议，MPC 保持当前安全设定值
              → SafetyAgent 生成最小风险状态方案
              → 输出结构化因果树（详见 §6.5.7）
```

#### 6.5.6.2 约束修剪与参考轨迹生成

当 LLM 建议通过黄区审核时，SafetyAgent 不是简单放行，而是执行**约束修剪**（Constraint Trimming）：将 LLM 建议中超出 ODD 正常域的部分"截断"到安全范围，再传递给 MPC 作为参考轨迹。

$$
\tilde{r}_i = \text{clip}(r_i^{\text{LLM}},\; l_i^{norm},\; u_i^{norm}) \tag{6-2}
$$

其中 $r_i^{\text{LLM}}$ 是 LLM 建议的目标值，$[l_i^{norm}, u_i^{norm}]$ 是 ODD 正常范围，$\tilde{r}_i$ 是修剪后传递给 MPC 的参考值。

修剪后的参考轨迹交给 MPC，MPC 在此基础上求解受约束的优化问题（求解方法详见 T2a 第七章 §7.3）：

$$
\min_{\mathbf{u}} \sum_{k=0}^{N_p - 1} \left\| \mathbf{y}(k) - \tilde{\mathbf{r}}(k) \right\|^2_{Q} + \left\| \Delta\mathbf{u}(k) \right\|^2_{R} \tag{6-3}
$$

$$
\text{s.t.} \quad \mathbf{y}(k) \in \mathcal{Y}_{ODD}^{norm}, \quad \mathbf{u}(k) \in \mathcal{U}_{safe}, \quad \forall\, k = 0, \ldots, N_p - 1
$$

其中 $\mathcal{Y}_{ODD}^{norm}$ 是 ODD 正常域对应的状态约束集，$\mathcal{U}_{safe}$ 是执行器安全动作集。约束在整个预测时域 $N_p$ 的每个步骤上均需满足（$\forall\, k$），而不仅仅在终端时刻——这确保了 MPC 规划的整个轨迹都保持在安全范围内，而非仅在当前时刻满足约束（MPC 的完整约束处理方法详见 T2a 第七章 §7.3.2）。

**关键设计原则**：MPC 层的约束必须比 ODD 正常域**更严格**（留有裕量），以防止 MPC 求解结果在数值误差下恰好落在 ODD 边界上。推荐裕量为 ODD 正常范围宽度的 5%-10%。

#### 6.5.6.3 实时一致性检查

由于 LLM 的慢思考建议是在某个时刻的状态快照下生成的，而执行时刻的系统状态可能已发生变化，SafetyAgent 在将建议传递给 MPC 前还需进行**实时一致性检查**：

```python
def check_temporal_consistency(self, llm_advice, current_state,
                                 advice_timestamp):
    """检查 LLM 建议的时效性"""
    age_seconds = time.time() - advice_timestamp
    state_drift = self.compute_state_drift(llm_advice.assumed_state,
                                            current_state)

    if age_seconds > self.config.advice_max_age_sec:
        return ConsistencyResult.STALE  # 建议过期，拒绝
    if state_drift > self.config.drift_threshold:
        return ConsistencyResult.INCONSISTENT  # 状态偏移过大，重新评估
    return ConsistencyResult.VALID  # 建议有效，可传递给 MPC
```

当建议过期或状态偏移过大时，SafetyAgent 通知 Orchestrator 重新触发 LLM 规划，而不是将过时建议强行传递给 MPC。

[图6-6: SafetyAgent 作为 MPC-LLM 安全接口的全链路架构]
{描述: 垂直三层图。顶层（慢思考，蓝紫色）：LLM/Agent 规划层，输出"方案级调度建议（参考轨迹+参数）"；中层（SafetyAgent，红色边框）：ODD 边界监测器，内含"实时一致性检查→ODD 三区间判定→约束修剪"三步流程；底层（快思考，蓝色）：MPC 控制器，输入修剪后的参考轨迹，输出"闸门/泵站执行指令"。中层三条出口分支：绿色"直接传递"、黄色"修剪后传递+人工确认"、红色"否决+输出结构化因果树"。整体布局从上到下，用垂直箭头连接，红色"否决"分支用粗线条强调。}
{尺寸: 全页}
{颜色方案: 蓝紫/红/蓝三层，红色边框突出 SafetyAgent}

### 6.5.7 结构化因果树：接管请求的规范化输出

> **五书协同注**：本节对接 T3 第13章"人工接管请求规范"。T3 规定：系统请求人类接管时，必须输出结构化因果树，不允许只发 Error 代码。本节给出认知AI层面的实现规范。

#### 6.5.7.1 为什么需要因果树

传统系统在请求人工接管时，通常只发送一条告警消息，如：

```
[ERROR] 水位越限，请操作员介入。Error Code: WL-OVERFLOW-003
```

这种方式存在根本缺陷：操作员面对一个 Error Code，不知道：
- **为什么会到这一步？**（是什么操作序列导致了当前状态？）
- **现在系统处于什么状态？**（除水位外，其他参数是否也异常？）
- **还有多少时间可以应对？**（预计多久后超出不可恢复的临界点？）
- **可以采取什么行动？**（有哪些应急方案可供选择？）

如果操作员在没有这些背景信息的情况下仓促决策，可能导致次生危险。结构化因果树正是解决"上下文缺失"问题的标准化方案。

#### 6.5.7.2 因果树的四要素结构

SafetyAgent 在触发接管请求时，必须同步输出以下四要素的结构化因果树：

```json
{
  "handover_request": {
    "request_id": "HR-20260303-142301",
    "timestamp": "2026-03-03T14:23:01+08:00",
    "urgency": "HIGH",

    "element_1_odd_trigger": {
      "description": "触发的ODD边界维度",
      "violated_dimensions": [
        {
          "dimension": "water_level_pool_3",
          "current_value": 4.15,
          "unit": "m",
          "normal_upper": 3.5,
          "extended_upper": 4.0,
          "status": "MRC_ZONE",
          "breach_magnitude": "+0.15m beyond extended boundary"
        },
        {
          "dimension": "inflow_rate_pool_3",
          "current_value": 0.18,
          "unit": "m3/s",
          "normal_upper": 0.10,
          "extended_upper": 0.15,
          "status": "MRC_ZONE",
          "breach_magnitude": "+0.03m3/s beyond extended boundary"
        }
      ]
    },

    "element_2_current_state": {
      "description": "系统当前状态快照",
      "timestamp": "2026-03-03T14:23:00+08:00",
      "pool_states": [
        {"pool_id": 3, "water_level": 4.15, "inflow": 0.18, "outflow": 0.09},
        {"pool_id": 4, "water_level": 2.8,  "inflow": 0.09, "outflow": 0.08}
      ],
      "gate_positions": {"G3-4": 0.35, "G4-5": 0.40},
      "active_alarms": ["WL_HIGH_POOL3", "INFLOW_HIGH_POOL3"],
      "mpc_last_action": "G3-4 reduced from 0.50 to 0.35 at 14:22:45"
    },

    "element_3_causal_chain": {
      "description": "触发前的最后3条控制指令（因果链）",
      "actions": [
        {
          "sequence": 1,
          "time": "2026-03-03T14:20:15+08:00",
          "initiator": "LLM_Planner",
          "action": "Increase inflow to pool 3 to Q=0.18 m³/s for silt flushing",
          "rationale": "Scheduled maintenance: silt accumulation in pool 3",
          "safety_check": "SafetyAgent approved (state was in normal zone at initiation)"
        },
        {
          "sequence": 2,
          "time": "2026-03-03T14:21:30+08:00",
          "initiator": "MPC",
          "action": "Attempted to reduce outflow G3-4 to compensate downstream level",
          "rationale": "MPC tracking downstream level setpoint",
          "note": "Conflict: high inflow + reduced outflow → accumulation accelerated"
        },
        {
          "sequence": 3,
          "time": "2026-03-03T14:22:45+08:00",
          "initiator": "SafetyAgent",
          "action": "Issued MRC action: G3-4 reduced to 0.35, inflow throttle requested",
          "rationale": "Water level entered extended zone, MRC protocol initiated",
          "result": "Level still rising due to upstream inertia"
        }
      ]
    },

    "element_4_time_budget": {
      "description": "预估剩余安全运行时间",
      "current_level": 4.15,
      "critical_threshold": 4.5,
      "current_rise_rate_m_per_min": 0.08,
      "estimated_time_to_critical_min": 4.4,
      "confidence": "MEDIUM",
      "estimation_basis": "conservative_worst_case",
      "note": "4.4 min is the CONSERVATIVE estimate assuming current inflow rate continues with no additional intervention. This is intentionally pessimistic (worst-case) to maximize operator response time. If upstream gate G2-3 closes now, actual time to critical extends to ~12 min.",
      "recommended_actions": [
        "Close upstream gate G2-3 immediately (fastest impact, ~2min response)",
        "Open emergency spillway SP3-E (available, 5min to full effect)",
        "Notify downstream operators to prepare for increased outflow"
      ]
    }
  }
}
...
因果树由 SafetyAgent 与 RAG/Report Agent 协同生成：

- **SafetyAgent 负责**：要素1（ODD触发维度）+ 要素2（当前状态快照）
- **执行日志模块负责**：要素3（最后3条控制指令）——从不可篡改的执行日志中回溯提取
- **物理预测模型负责**：要素4（剩余安全时间）——调用 MPC 的预测能力，以当前状态为初始条件，估算在"无进一步干预"假设下达到临界值的时间

Report Agent 将四要素整合为自然语言摘要，与结构化 JSON 一起推送给操作员界面：

```
【接管请求 HR-20260303-142301】
紧急程度：HIGH | 触发时间：14:23:01

核心问题：3号渠池水位（当前 4.15m）已超出扩展域上限（4.0m），
         入流量（0.18 m³/s）超出安全上限（0.15 m³/s）

直接原因：14:20 执行 LLM 建议的冲淤作业，入流增大后 MPC 同时
         收紧了下游闸门，导致 3 号渠池来多去少，水位快速上升。

剩余时间：约 4.4 分钟后水位达到 4.5m 临界值（中等置信度）
...
[图6-7: 结构化因果树输出格式与生成机制]
{描述: 左侧：因果树四要素（ODD触发、当前状态、因果链、时间预算），每个要素用不同颜色（红/橙/黄/绿）标注；右侧：生成流程图，展示SafetyAgent/日志模块/物理预测模型/Report Agent的分工协作；中间：操作员界面上的自然语言摘要示例。}
{尺寸: 全页}

### 6.5.8 Agent 间通信机制：MessageBus 与协议
...
[图6-6: 认知API四维框架与角色映射]
{描述: 矩阵图。横轴四列：感知/认知/决策/控制。纵轴五行：operator/designer/researcher/admin/teacher。每个单元格用颜色标注可访问(蓝色)或不可访问(灰色)。admin行全蓝。右侧标注CHS理论的四维认知链路方向。}
{尺寸: 半页}
{颜色方案: 蓝色可访问/灰色不可访问}
...
[图6-7: 双引擎闭环原理（简洁版）]
{描述: 圆形闭环流程图。上半部分紫色区域"认知AI引擎"：理解→推理→提议方案。下半部分蓝色区域"物理AI引擎"：约束检查→水力仿真→校验结果。左侧：提议方案从认知传向物理（↓）。右侧：校验结果从物理传回认知（↑），有两条路径——通过（绿色→执行）和否决（红色→修改方案）。中间菱形决策节点"校验通过?"。底部标注："物理AI拥有绝对否决权"。}
{尺寸: 半页}
{颜色方案: 紫色(认知)+蓝色(物理)+绿色(通过)+红色(否决)}

---

## 6.9 自进化管线：影子模式与 OTA 升级

> **五书协同注**：本节对接 T3 第10章"OTA升级审查流程"。T3 规定：任何影响物理控制网络的策略更新，必须经过影子模式验证和人类审计，方可并网。本节从认知AI角度给出影子模式管线的设计原理与实现规范。

### 6.9.1 为什么认知AI需要"影子模式"

认知AI（LLM 驱动的 Agent）具备学习新策略的能力——通过分析历史调度数据、运维反馈和专家评审，系统可以不断更新其调度建议模型。然而，将新学到的策略**直接写入物理控制网络**存在极高风险：

**风险一：分布偏移（Distribution Shift）**：训练数据覆盖的工况可能不包含某些极端组合。新策略在训练集上表现优异，但在未见过的工况下可能产生危险决策。

**风险二：目标偏移（Objective Misalignment）**：认知AI优化的目标函数（如"最小化调度响应时间"）可能与实际工程目标（如"最小化溢坝风险"）之间存在微妙偏差，长期累积后可能导致策略漂移。

**风险三：系统干扰（System Interference）**：新策略可能与现有 MPC 控制器的行为产生意外耦合，导致控制环路振荡或稳定性下降。

**影子模式**（Shadow Mode）是应对上述风险的标准隔离机制：新策略在**不发出实际控制指令**的情况下并行运行，仅记录"如果执行此策略，建议是什么"，与实际执行结果对比。只有通过充分验证后，才允许策略并网。
...
```
阶段 1：影子并行运行（Shadow Execution）
─────────────────────────────────────
实际控制链: 传感器 → MPC/现有策略 → 执行器（实际指令）
                         │
影子链:   传感器 → 新策略AI Agent → 影子建议（不执行）
                         │
记录:     实际执行结果 vs 影子建议 → 对比日志

阶段 2：人类审计（Human Audit）
─────────────────────────────
审计员访问对比日志，评估：
  - 影子建议与实际执行的一致性（%）
  - 影子建议在 ODD 安全范围内的比例（%）
...
阶段 3：OTA 升级并网（Over-The-Air Deployment）
──────────────────────────────────────────────
审计通过 → 生成升级包
OTA 升级: 灰度发布（先部署到 5% 的边缘节点）
灰度期（7天）无异常 → 全量部署
全量部署后继续影子监控 30 天（降级保护期）
```

### 6.9.3 影子模式的 60 天验证标准

T3 第10章规定的影子模式验证周期为 **60 天**，对应以下量化门禁：

| 评估指标 | 最低标准 | 说明 |
|---------|---------|------|
| ODD 合规率 | ≥ 99.5% | 影子建议中，处于 ODD 正常域内的比例 |
| 一致性率 | ≥ 85% | 影子建议与实际执行的方向一致比例（调大/调小方向相同） |
| MRC 偏差 | = 0 次 | 影子建议与实际执行在 MRC 触发判定上必须完全一致 |
| 极端场景覆盖 | ≥ 3 次 | 60天内必须覆盖至少 3 次极端工况（洪水、检修、设备故障等） |
| 人类否决率 | ≤ 2% | 审计员人工标注"此建议不可接受"的比例 |

**重要约束**：影子模式期间，新策略 Agent **绝对不允许**：
- 直接向执行器发送任何指令
- 修改 MPC 的参数或约束
- 覆盖 SafetyAgent 的 ODD 判定结果

违反上述约束的任何尝试，必须触发审计告警并暂停影子运行。

### 6.9.4 灰度发布与降级保护

OTA 升级采用灰度发布策略，防止全量部署后的大规模失效：
...
class OTADeployment:
    ...
    PHASES = [
        DeployPhase(name="canary",     nodes_pct=5,   duration_days=7,
                    rollback_threshold=0.01),   # 异常率 >1% 自动回滚
        DeployPhase(name="early",      nodes_pct=20,  duration_days=7,
                    rollback_threshold=0.005),
        DeployPhase(name="majority",   nodes_pct=50,  duration_days=14,
                    rollback_threshold=0.002),
        DeployPhase(name="full",       nodes_pct=100, duration_days=None,
                    rollback_threshold=0.001),  # 全量：长期监控
    ]

    def advance_phase(self, current_phase: DeployPhase,
                      anomaly_rate: float) -> str:
        if anomaly_rate > current_phase.rollback_threshold:
            self.rollback(current_phase)
            return "ROLLED_BACK"
        if self.phase_duration_met(current_phase):
            return self.deploy_next_phase()
        return "MONITORING"
```

**降级保护期**：全量部署后 30 天内，旧版本策略保持热备，一旦新版本触发 ODD MRC 告警超过阈值（默认连续 3 次），系统自动切回旧版本并锁定，等待人工审查。

### 6.9.5 自进化管线与 HydroOS 工作台的集成

自进化管线作为 HydroOS 工作台层（详见第九章 §9.5）的一个子系统，与记忆模块和 RBAC 模块深度集成：

- **记忆模块**为影子运行提供历史对比基准，记录"策略版本→运行期间表现"的完整时序
- **RBAC 模块**限制只有 `admin` 角色可以触发 OTA 并网审批，`operator` 角色只能查看影子运行报告
- **心跳模块**确保影子运行中断（如网络故障）时自动暂停计时，不将故障期间的空白纳入 60 天有效期

[图6-8: 自进化管线三阶段架构]
{描述: 水平三阶段流程图。第一阶段（蓝色）"影子并行运行"：实际控制链（实线）和影子链（虚线）并行，对比日志存储于中间数据库。第二阶段（橙色）"60天人类审计"：审计员界面展示对比报告，五项门禁指标逐一核查，通过✓/不通过✗标注。第三阶段（绿色）"OTA灰度发布"：5%→20%→50%→100%节点渐进部署，每步有回滚阈值。各阶段之间用箭头连接，失败路径用红色虚线标注回滚方向。}
{尺寸: 全页}
{颜色方案: 蓝色(影子)/橙色(审计)/绿色(部署)/红色(回滚)}
...
8. **SafetyAgent 与 ODD**：三区间判定（正常→放行、扩展→人工确认、MRC→否决）是安全保障的核心。ODD 是多维安全空间（水位、流量、水温、传感器状态等），三区间判定公式 (6-1) 给出了严格的数学定义。SafetyAgent 拥有绝对否决权，高于所有 Agent。**SafetyAgent 同时充当 MPC 快思考与 LLM 慢思考之间的安全接口（ODD 边界监测器）**：通过实时一致性检查、约束修剪（式6-2）和时序有效性验证三步过滤，确保 LLM 调度建议在传递给 MPC 前经过安全验证。接管请求必须输出结构化因果树（四要素：ODD 触发维度、当前状态、因果链、剩余安全时间），支持操作员快速决策。

9. **自进化管线**：新策略必须先在影子模式运行 60 天，满足五项量化门禁（ODD 合规率≥99.5%、一致性率≥85%、MRC 偏差=0、极端场景≥3次、人类否决率≤2%），通过人类审计后才允许 OTA 灰度发布并网（5%→20%→50%→100%），全量部署后保留 30 天降级保护期。

10. **认知API 四维框架**：感知→认知→决策→控制，与 CHS 理论对应。五种用户角色（operator/designer/researcher/admin/teacher）通过 RBAC 三级粒度（EXECUTE/READ/DENIED）权限矩阵实现按需访问。统一网关将自然语言映射到认知API维度。

11. **双引擎闭环**：认知AI提议→物理AI校验→执行。物理AI拥有绝对否决权，基于经过验证的数学模型（Saint-Venant方程、质量守恒方程），是安全运行的最后防线。双引擎闭环在工业水网中呈现更高的迭代频率和更复杂的工艺耦合特点。
...
| ch09（HydroOS） | 五层架构在HydroOS操作系统中的集成方式；自进化管线与工作台层的集成 |
| ch10（在环测试） | Skill的MIL/SIL/HIL验证方法论 |
| T2a ch10（安全包络） | ODD三区间否决权的物理AI理论基础 |
| T2a ch12（分层控制） | 物理AI引擎的HDC控制架构；MPC快思考执行层的理论基础 |
| T3 ch8（ODD硬件规范） | SafetyAgent作为ODD边界监测器的工程对标 |
| T3 ch10（OTA升级审查） | 影子模式60天验证与灰度发布规范的技术来源 |
| T3 ch13（接管请求规范） | 结构化因果树输出格式的合规要求 |
