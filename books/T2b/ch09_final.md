...
> **【HydroOS 与 HydroClaw 的关系】**
> 本节以 HydroClaw v0.2.2 作为 HydroOS 的参考实现示例。需要特别说明的是，HydroClaw 是 CHS 理论体系的**初版原型**（initial prototype）系统，其工作台层的人格系统、记忆管理、RBAC 权限控制等五大子系统代表了 HydroOS 的理论方向，但具体实现（代码架构、数据格式、接口规范等）仍在持续演进。工程应用时应借鉴其架构模式和设计原则，根据具体场景进行二次开发，而非直接部署原型代码到生产环境。
...
| 控制 | 闸门调节、泵站启停、MRS 触发 | 调度方案 | 执行结果、效果评估 |
...
工作内容：部署 SafetyAgent 和 ODD 边界管理，在 ODD 绿区内允许系统自主下发控制指令，黄区需人工确认，红区自动触发 MRS。搭建 HiL 验证环境。
